...
When contributing, keep in mind that Monis Agent customers (that's you!) are running many different versions of the Node.js agent, some of them pretty old. Changes that depend on the newest version of the Node.js agent will probably be rejected, especially if they replace something backwards compatible.
