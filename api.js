...
function API(agent) {
  this.agent = agent
  this.shim = new TransactionShim(agent, 'MonisAgentAPI')
...
/**
 * Sends an application log message to Monis Agent. The agent already
...
/**
 * Record custom event data which can be queried in Monis Agent Insights.
 *
 ...
 * Registers an instrumentation function.
 *
 * - `monisagent.instrumentConglomerate(moduleName, onRequire [, onError])`
 * - `monisagent.instrumentConglomerate(options)`
 *
 * @param {string|object} moduleName The module name given to require to load the module, or the instrumentation specification
 * @param {string} moduleName.moduleName The module name given to require to load the module
 * @param {string} [moduleName.absolutePath] Must provide absolute path to module if it does not exist within node_modules. This is used to instrument a file within the same application.
...
/**
 * Record a LLM feedback event which can be viewed in Monis Agent API Monitoring.
 *
 ...
 *   }
 * }
 * monisagent.setErrorGroupCallback(myCallback)
 */
...
/**
 * Ignores the current transaction when calculating your {@link https://docs.monisagent.com/docs/apm/monis-agent-apm/apdex/apdex-measuring-user-satisfaction/|Apdex score}.
