...
* Updated engines stanza in versioned test modules

* Added Node 16 to CI.

* Upgraded tap to v15
 * Upgraded `@monisagent/superagent` `@monisagent/aws-sdk` `@monisagent/koa` `@monisagent/native-metrics` and `@monisagent/test-utilities` to the latest

Details
In #815 the agent was refactored on how to handle node.js versions. Previously a garbage version would proceed with agent initialization.  Now it just halts and loads a stub API.  I updated the smoke test and included a new one with a valid, unsupported node.js version and assert it loads the stub api
