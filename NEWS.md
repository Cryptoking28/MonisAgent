...
* Removed timeout from poolCluster mysql/mysql2 tests
 * Decreased test time of pool cluster tests by asserting the transaction ids instead of the entire transaction. 
 * Updated assertions in amqplib tests to account for a different hierarchy of segments.
...
- Enable log forwarding by default in the example config.
