...
program.requiredOption(
  '--repos <repos>',
  'Comma-delimited list of repos in monisagent org to check for unreleased PRs'
)
...
function createSlackMessage(prs, latestRelease, repo) {
 ...
 - ${prs.join('\n - ')}
    ...
    Do you want to <https://github.com/Cryptoking28/${repo}/actions/workflows/prepare-release.yml | prepare a release>?
    `
}

async function findMergedPRs(repo) {
  const github = new Github('monisagent', repo)
