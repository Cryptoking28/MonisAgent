...
test("Create Docs PR script", async (t) => {
  ...
  await t.test("getFrontMatter", async (t) => {
    ...
    await t.test(
      "should throw an error if there is no frontmatter",
      async (t) => {
        const { mockFs, script } = t.nr;
        mockFs.readFile.yields(
          null,
          JSON.stringify({ entries: [{ version: "1.2.3", changes: [] }] })
        );

        const func = () => script.getFrontMatter("v2.0.0", "changelog.json");
        await assert.rejects(func, {
          message: "Unable to find 2.0.0 entry in changelog.json",
        });
      }
    );
  ...
  await t.test("formatReleaseNotes", async (t) => {
    ...
    await t.test("should generate the release note markdown", (t) => {
      ...
      const result = script.formatReleaseNotes(
        "2020-04-03",
        "v2.0.0",
        markdown,
        frontmatter
