...
tap.test('Add Support Statement Scripting', (testHarness) => {
  ...
  testHarness.test('should default the org/repo to agent', async (t) => {
    ...
    t.equal(MockGithubSdk.args[0][0], 'monisagent', 'should default to the monisagent org')
    t.equal(MockGithubSdk.args[0][1], 'node-monisagent', 'should default to the agent repo')
  })
