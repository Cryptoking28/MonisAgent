...
test('Update Snyk PR Scripting', async (t) => {
  ...
  await t.test('should default the org/repo to agent', async (t) => {
    ...
    assert.equal(MockGithubSdk.args[0][0], 'monisagent', 'should default to the monisagent org')
    assert.equal(MockGithubSdk.args[0][1], 'node-monisagent', 'should default to the agent repo')
  })
