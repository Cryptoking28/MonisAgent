...
program.requiredOption(
  "--tag <tag>",
  "tag name to get version of released agent"
);
program.option("--remote <remote>", "remote to push branch to", "origin");
program.option(
  "--username <github username>",
  "Owner of the fork of docs-website"
);
...
program.option(
  "--dry-run",
  "executes script but does not commit nor create PR"
);
program.option(
  "--repo-path <path",
...
async function createReleaseNotesPr() {
  ...
  GITHUB_USER =
    options.username || process.env.GITHUB_USER || process.env.GITHUB_ACTOR;
  GITHUB_EMAIL =
    options.email ||
    process.env.GITHUB_EMAIL ||
    `gh-actions-${GITHUB_USER}@github.com`;
  ...
  try {
    ...
    const releaseNotesBody = formatReleaseNotes(
      releaseDate,
      version,
      body,
      frontmatter
...
async function getFrontMatter(tagName, frontMatterFile) {
  ...
  const frontmatter = changelog.entries.find(
...
async function createPR(version, branch, dryRun, repoOwner) {
  ...
  const head = repoOwner === "monisagent" ? branch : `${repoOwner}:${branch}`;
  const body =
