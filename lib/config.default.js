...
exports.config = {
   ...
   * disallow the use of Monis Agent's server-side configuration for agents. To
   * do so, set this parameter to true. Some configuration information is
   * required to make the agent work properly with the rest of Monis Agent, but
   ...
   * is more latency-sensitive than many environments, but Monis Agent's standard
   * is 0.5 seconds.
   *
   * @env NEW_RELIC_APDEX
   */
  ...
  transaction_tracer : {
     ...
     * traces faster, but it increases the volume of data sent to Monis Agent, which
