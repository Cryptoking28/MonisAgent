...
module.exports = {
  ...
  copyParameters: function copyParameters(config, source, destination) {
    var attributesEnabled = (config && config.attributes && config.attributes.enabled)
    if (!attributesEnabled || !source || !destination) {
      return
    }
    ...
    for (var i = 0; i < keys.length; i++) {
      var key = keys[i]
      if (config.attributes.exclude.indexOf(key) === -1 && !(key in destination)) {
   ...
   * `attributes.enabled` is not set to true.
   *
   * @param {Config} config      Configuration, where `ignored_params` is
   *                             guaranteed to be an Array.
   * @param {object} source      Parameters to be copied (not changed).
  ...
  overwriteParameters: function overwriteParameters(config, source, destination) {
    ...
    for (var i = 0; i < keys.length; i++) {
      var key = keys[i]
      if (config.attributes.exclude.indexOf(key) === -1) {
        destination[key] = source[key]
      }
    }
  }
