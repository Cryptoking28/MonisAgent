...
function Stats() {
	...
	this.recordValueInMillis = function(totalTime, exclusiveTime) {
		this.recordValue(totalTime / 1000, exclusiveTime ? exclusiveTime / 1000 : null);
	}
...
function StatsEngine(logger) {
	...
	this.getMetricData = function() {
		return new MetricDataSet(unscopedStats, scopedStats, metricIds);
	}
	
	this.parseMetricIds = function(metricIdArray) {
		lastSendTime = getTime();
		for (var index in metricIdArray) {
			var idToSpec = metricIdArray[index];
			var spec = idToSpec[0];
			var id = idToSpec[1];
	...
	this.mergeMetricData = function(metricDataSet) {
		...
		for (var scope in metricDataSet.scopedStats) {
			this.getScopedStats(scoped).merge(metricDataSet.scopedStats[scope]);
		}
	}
