  ...
  _setupSpanStreamObservers(stream) {
    ...
    stream.on('status', (grpcStatus) => {
      ...
      const metric = this._metrics.getOrCreateMetric(
        util.format(NAMES.INFINITE_TRACING.SPAN_RESPONSE_ERROR_STATUS, grpcStatusName)
      )
      ...
      if (grpc.status[grpc.status.UNIMPLEMENTED] === grpcStatusName) {
        const unimplementedMetric = this._metrics.getOrCreateMetric(
          NAMES.INFINITE_TRACING.SPAN_RESPONSE_UNIMPLEMENTED
        )
        unimplementedMetric.incrementCallCount()
