...
class GrpcConnection extends EventEmitter {
  ...
  connectSpans() {
    ...
    setTimeout(() => {
      try {
        ...
        setTimeout(
          // create a version of the method with
          this.connectSpans,
  ...
  _setupSpanStreamObservers(stream) {
    ...
    stream.on('status', (grpcStatus) => {
      ...
      // TODO: all "error" statuses need a supportability  metric

      // all statuses treated as "bad stuff" happened, we need
      // to reconnect.  Event an "OK status" indicates the call
      // completed succesfully, which indicates the stream is
      // over.
      if ('UNIMPLEMENTED' === grpc.status[grpcStatus.code]) {
