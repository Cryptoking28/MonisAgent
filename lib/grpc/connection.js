...
class GrpcConnection extends EventEmitter {
  ...
  _getMetadata(license_key, run_id, requestHeadersMap, env) {
    ...
    this._getTestMetadata(metadata, env)
   ...
   */
  _getTestMetadata(metadata, env) {
    for (const key in GRPC_TEST_META) {
      const value = parseInt(env[GRPC_TEST_META[key]], 10)
      if (value) {
  ...
  _createClient(endpoint) {
    ...
    const traceApi = protoDescriptor.com.monisagent.trace.v1
