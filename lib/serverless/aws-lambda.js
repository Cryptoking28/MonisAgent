  ...
  patchLambdaHandler(handler) {
    ...
    if (typeof handler !== 'function') {
      ...
      function wrappedHandler() {
        ...
        const awsAttributes = getAwsAgentAttributes(awsLambda, event, context)
        ...
        if (!shim.agent.lambdaArn) {
          // Pull the ARN off the context to a well known place
          shim.agent.lambdaArn = context.invokedFunctionArn
        }
...
function getAwsAgentAttributes(awsLambda, event, context) {
  const attributes = {
    all: {
