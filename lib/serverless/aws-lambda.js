...
const EVENT_TYPE_INFO = {
  ...
  "cloudWatch_scheduled": {
    ...
    "required_keys": [["source"], ["detail-type"]]
  },
  "dynamo_streams": {
    "name": "dynamo_streams",
  ...
  "firehose": {
    "name": "firehose",
    "required_keys": [["deliveryStreamArn"], ["records", 0, "kinesisRecordMetadata"]]
  },
  "kinesis": {
    "name": "kinesis",
...
class AwsLambda {
  ...
  detectEventType(event) {
    ...
    for (const typeInfo of Object.values(EVENT_TYPE_INFO)) {
      if (typeInfo.required_keys.every((path) => pathMatch(path, event))) {
        return typeInfo
      }
    }
    return null
  ...
  _getAwsAgentAttributes(event, context) {
    ...
    const eventTypeInfo = this.detectEventType(event)
    if (eventTypeInfo) {
      attributes[EVENT_SOURCE_TYPE_KEY] = eventTypeInfo.name
    }
...
function setWebRequest(shim, transaction, request) {
  ...
  if (shim.agent.config.distributed_tracing.enabled) {
    // W3C Fork based on format
    if (shim.agent.config.feature_flag.dt_format_w3c) {
      ...
      if (parent) {
        ...
        transaction.acceptTraceContextPayload(parent)
      }
    } else {
      ...
      const payload = request.headers.monisagent || request.headers.NEWRELIC ||
