  ...
  patchLambdaHandler(handler) {
    ...
    async function wrappedHandler() {
      ...
      if (shim.isPromise(res)) {
        res = await lambdaInterceptPromise(res)
      }
      return res
    }

    // copy of shim.interceptPromise function but async so that we can await errors
    async function lambdaInterceptPromise(prom, cb) {
      if (shim.isFunction(prom.finally)) {
