...
function Agent(config) {
  ...
  // This starts at 10, but after the first harvest it goes to 2 * sampledTarget
  this.maxSampled = this.sampledTarget
 ...
 * by the agent.
 */
Agent.prototype.adjustDistributedTraceStats = function adjustDistributedTraceStats() {
  if (this.transactionCreatedInHarvest) {
    // set the min to be 0 (take all the transactions) if we haven't hit
    // the target
    var samplingRatio = Math.min(this.sampledTarget / this.transactionCreatedInHarvest, 1)
    this.minSampledPriority = 1 - samplingRatio
  }

  // This is currently a constant since this.sampledTarget is not
  // configurable.
  this.maxSampled = 2 * this.sampledTarget
}
...
function shouldSampleDistributedTrace(txn) {
  const sampled = this.transactionsSampled < this.maxSampled
    && txn.priority >= this.minSampledPriority
  if (sampled) {
