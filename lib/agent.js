...
Agent.prototype._sendEvents = function _sendEvents (callback) {
  if (this.config.analytics_events.enabled) {
    var agent  = this;
    ...
    // TODO: re-aggregate events on failure
    agent.collector.analyticsEvents(payload, callback);
  }
  else {
    process.nextTick(callback);
  }
...
Agent.prototype._addEventFromTransaction = function (transaction) {
  if (!this.config.analytics_events.enabled) return;

  var maxSize = this.config.analytics_events.max_samples_per_minute;
  var custom  = transaction.getTrace().custom;
  var event = [
    {
      "webDuration" : transaction.timer.duration,
      "timestamp"   : transaction.timer.start,
      "name"        : transaction.name,
      "duration"    : transaction.timer.duration,
      "type"        : "Transaction"
    },
    custom
  ];

  // TODO: add sampling on spillage
  var spilled = this.events.length >= maxSize;
  if (spilled) logger.warn('Event Spillage', event);
