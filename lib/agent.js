...
Agent.prototype.start = function start(callback) {
  ...
  if (this.config.lambda_mode) {
    return this._lambdaModeStart(callback)
  }
 ...
 */
Agent.prototype._lambdaModeStart = function _lambdaModeStart(callback) {
  const agent = this

  logger.info(
    'Monis Agent for Node.js starting in AWS Lambda -- skipping connection process.'
  )
