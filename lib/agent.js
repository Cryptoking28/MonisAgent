...
function _addIntrinsicAttrsFromTransaction(transaction) {
  ...
  if (this.config.cross_application_tracer.enabled) {
    if (this.config.feature_flag.distributed_tracing) {
      // TODO: pull out the relevant data from https://source.datanerd.us/earnold/agent-specs/blob/dd291549d8af103d64ee6fba061a3c19cd23cc70/Distributed-Tracing.md#attributes
    } else if (
...
Agent.prototype.recordSupportability = function recordSupportability(name, value) {
  var metric = this.metrics.getOrCreateMetric(NAMES.SUPPORTABILITY + '/' + name)
  if (value != null) {
