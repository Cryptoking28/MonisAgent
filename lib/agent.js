...
Agent.prototype.start = function () {
  if (this.config.agent_enabled !== true) {
    return logger.warn("The Monis Agent Node.js agent is disabled by its configuration. " +
                       "Not starting!");
  }
  ...
  if (!this.applicationPort) {
    logger.debug("No application port found. Waiting 15 seconds to connect.");
    setTimeout(this.emit.bind(this, 'ready'), 15 * 1000);
  }
  else {
    this.emit('ready');
  }
...
Agent.prototype.stop = function () {
  ...
  logger.info("Monis Agent Node.js instrumentation stopped.");
};

Agent.prototype.restart = function () {
  logger.info("Resetting the agent's connection to Monis Agent.");
...
Agent.prototype.onApdexTChange = function (params) {
  ...
  if ((params.apdex_t || params.apdex_t === 0) &&
    ...
    logger.info("Apdex tolerating value changed from %d to %d.",
                this.apdexT,
                params.apdex_t);
 ...
 */
Agent.prototype.onNormalizationChange = function (response) {
  this.normalizer.load(response);
};

Agent.prototype.onNewRenameRules = function (metricIDs) {
  if (!metricIDs) {
...
Agent.prototype.setupConnection = function () {
  ...
  this.connection.on('connect',            this.onNormalizationChange.bind(this));
  ...
  /* Handle the initial connection attempt specially for the application port.
   * Once the connection workflow has started, ignore further readiness events.
   */
  this.once('ready', this._connect.bind(this));
};
...
Agent.prototype._connect = function () {
  // wait until agent's had a chance to snarf a service port to set ID
  this.setID();
...
Agent.prototype._failAndShutdown = function (data, error) {
  ...
  logger.error("Unable to connect to Monis Agent after %s attempts.",
               this.connectionFailures + 1);
...
Agent.prototype.setID = function () {
  this.id = this.config.applications() + ':nodejs:' + os.hostname();
  if (this.applicationPort) this.id += ':' + this.applicationPort;
  logger.info('This agent\'s Monis Agent ID is "%s".', this.id);
};
