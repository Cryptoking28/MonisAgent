...
function Agent(config) {
  ...
  // The target will become configurable by the server in the future.
  // DT sampling will not be tied to the harvest time, and will have its own period
  // for sampling.
  this.sampledTarget = 10
  this.maxSamples = 2 * this.sampledTarget
...
Agent.prototype.incrementTransactionsSampled = function incrementTransactionsSampled() {
  ...
  if (this.transactionsSampled > this.sampledTarget) {
    ...
    if (this._lastHarvest === null) {
      ...
      var thresholdMax = this.sampledTarget / this.maxSamples
      var target = this.sampledTarget
      adjustedTarget = Math.pow(target, ratio) - Math.pow(target, thresholdMax)
    }
