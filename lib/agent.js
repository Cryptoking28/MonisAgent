  ...
  if (this.collector.isConnected()) {
    this.collector.shutdown(function (error) {
      if (error) {
        ...
        logger.info("Connection to Monis Agent's servers closed.");
      }
...
/**
 * Reconnect to Monis Agent, ignoring any disconnection failures (except
...
Agent.prototype.restart = function (callback) {
  ...
  logger.info("Reconnecting to Monis Agent.");
  this.stop(function (stopError) {
    ...
    agent.start(function (startError) {
      logger.info("Reconnected to Monis Agent.");
...
Agent.prototype._startHarvester = function (harvestSeconds) {
  ...
  function onError(error) {
    if (error) {
      logger.warn(error, "Error on submission to Monis Agent (data held for redelivery):");
    }
  }
