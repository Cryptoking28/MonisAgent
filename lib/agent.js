...
Agent.prototype.start = function start(callback) {
  ...
  if (this.config.serverless_mode) {
...
Agent.prototype._serverlessModeStart = function _serverlessModeStart(callback) {
  ...
  process.nextTick(() => callback(null, agent.config))
}
...
Agent.prototype.harvest = function harvest(callback) {
  ...
  collector.preparePayload && collector.preparePayload()

  // Send the harvest!
  this._lastHarvest.send(function afterHarvest(err) {
    if (err) {
      return callback(err)
    }
    collector.flushPayload(callback)
  })
}
...
function _addIntrinsicAttrsFromTransaction(transaction) {
  ...
  const coldStartMetric = transaction.metrics.getMetric('coldStartTime')
  if (coldStartMetric) {
    intrinsicAttributes.coldStartTime = coldStartMetric.total
  }
 ...
 */
Agent.prototype._transactionFinished = function _transactionFinished(transaction, cb) {
