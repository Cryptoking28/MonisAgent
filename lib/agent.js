...
Agent.prototype.adjustTransactionStats = function adjustTransactionStats(target) {
  if (this.transactionCreatedInHarvest) {
    target = target == null ? this.sampledTarget : target
...
Agent.prototype.incrementTransactionsSampled = function incrementTransactionsSampled() {
  ...
  if (this.transactionsSampled > this.sampledTarget) {
    var adjustedTarget
    if (this._lastHarvest === null) {
      // For the first harvest we only want to set sampled = true on the first ten
      // transactions.
      adjustedTarget = 0
    } else {
      var ratio = this.sampledTarget / this.transactionsSampled
      var target = this.sampledTarget
      ...
      // are using this constant.  In the interest of consistency we will use the
      // constant as well.
      adjustedTarget = Math.pow(target, ratio) - Math.pow(target, 0.51)
    }
    this.adjustTransactionStats(adjustedTarget)
  }
}
