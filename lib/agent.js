...
function Agent(config) {
  ...
  if (config.feature_flag.infinite_tracing && config.infinite_tracing) {
    if (!config.infinite_tracing.trace_observer) {
      // Trace observer is not set, send support metric and warn user
      this.metrics.getOrCreateMetric(
        NAMES.INFINITE_TRACING.MALFORMED_TRACE_OBSERVER
      ).incrementCallCount()
      logger.warn('Infinite tracing disabled: `trace_observer` must be set')
    } else if (!psemver.satisfies('>=10.10.0')) {
      logger.warn(
        'Infinite tracing disabled: this version of Node is not supported (must be >=10.10.0)'
      )
    } else if (config.serverless_mode.enabled) {
      logger.warn(
        'Infinite tracing disabled: serverless mode in the node agent not supported'
      )
    } else {
      ...
      this.spanEventAggregator = new StreamingSpanEventAggregator(
        {
          periodMs: 1000,
          limit: 50000,
