...
Agent.prototype._listenForConfigChanges = function _listenForConfigChanges() {
  ...
  this.config.on('sampling_target', function updateSamplingTarget(key, target) {
    self.transactionSampler.samplingTarget = target
  })
  this.config.on(
    'sampling_target_period_in_seconds',
    function updateSamplePeriod(key, period) {
      self.transactionSampler.samplePeriod = period * 1000
    }
  )
}
