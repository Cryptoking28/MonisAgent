...
Agent.prototype.restart = function () {
  logger.info("Restarting the agent's connection to Monis Agent.");
...
Agent.prototype.connect = function () {
  if (!this.applicationPort) {
    logger.debug("No listeners detected. " +
                 "Waiting another 15 seconds before finishing startup.");
...
Agent.prototype.updateRenameRules = function (metricIDs) {
  if (!metricIDs) {
    return logger.warn('Unable to update metric renaming rules: no new rules passed in.');
  }
 ...
 */
Agent.prototype.scheduleRetry = function (data, error) {
...
Agent.prototype.collectorSetup = function () {
  ...
  // we know enough at this point to set the agent's ID
