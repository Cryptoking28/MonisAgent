...
Agent.prototype.start = function start(callback) {
  ...
  if (!semver.satisfies(process.version, '~0.10')) {
    var proxyInUse = agent.config.proxy || agent.config.proxy_host || agent.config.proxy_port
    if (proxyInUse) {
      logger.error('Proxies are only supported on ~0.10 currently.')
      this.state('errored')
      return process.nextTick(function cb_nextTick() {
        callback(new Error('Proxies are only available on node ~0.10.'))
      })
    }
  }
...
Agent.prototype._transactionFinished = function _transactionFinished(transaction) {
  ...
  if (!transaction.ignore) {
    ...
    // FLAG: synthetics
    if (this.config.feature_flag.synthetics) {
      var data = transaction.syntheticsData

      if (data) {
        trace = transaction.getTrace()
        trace.intrinsics['synthetics_resource_id'] = data.resourceId
        trace.intrinsics['synthetics_job_id'] = data.jobId
        trace.intrinsics['synthetics_monitor_id'] = data.monitorId
      }
    }
