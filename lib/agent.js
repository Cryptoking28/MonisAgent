...
const AttributeHelper = require('./util/attributes')
...
Agent.prototype.start = function start(callback) {
  ...
  if (this.config.agent_enabled !== true) {
    logger.warn('The Monis Agent Node.js agent is disabled by its configuration. ' + 'Not starting!')
...
/**
 * Completes any final setup upon full connection to Monis Agent
...
Agent.prototype._serverlessModeStart = function _serverlessModeStart(callback) {
  logger.info('Monis Agent for Node.js starting in serverless mode -- skipping connection process.')
...
function _addIntrinsicAttrsFromTransaction(transaction) {
  ...
  AttributeHelper.maybeAddQueueAttributes(transaction, intrinsicAttributes)
  AttributeHelper.maybeAddExternalAttributes(transaction, intrinsicAttributes)
  AttributeHelper.maybeAddDatabaseAttributes(transaction, intrinsicAttributes)
  AttributeHelper.maybeAddSyntheticAttributes(transaction, intrinsicAttributes)

  if (this.config.distributed_tracing.enabled) {
    transaction.addDistributedTraceIntrinsics(intrinsicAttributes)
    AttributeHelper.maybeAddParentAttributes(transaction, intrinsicAttributes)
  } else if (
    ...
    AttributeHelper.addRequiredCATAttributes(transaction, intrinsicAttributes, this.config)
    AttributeHelper.maybeAddExtraCATAttributes(transaction, intrinsicAttributes, this.config)
  }
