...
const SpanStreamer = require('./spans/span-streamer')
...
function Agent(config) {
  ...
  // TODO: cleanup aggregator creation/usage
  if (config.feature_flag.streaming_spans) {
    // loading GrpcConnection behind a feature flag
    // to avoid version complaints
    ...
    // TODO: handle grpc-js supported versions.
    // Requiring this will load grpc-js which will throw for unsupported versions.
    const StreamingSpanEventAggregator = require('./spans/streaming-span-event-aggregator')
    this.streamingSpanEventAggregator = new StreamingSpanEventAggregator(
      {
        agent:this,
        periodMs: 1000,
        limit: 50000,
...
Agent.prototype.startAggregators = function startAggregators() {
  ...
  // TODO: cleanup aggregator creation/usage so consumers don't
  // need to know about both.
  if (this.config.distributed_tracing.enabled &&
      this.config.span_events.enabled) {
    if (this.config.feature_flag.streaming_spans && this.streamingSpanEventAggregator) {
      logger.debug('starting streamingSpanEventAggregator')
      this.streamingSpanEventAggregator.start()
    } else {
...
Agent.prototype._serverlessModeStart = function _serverlessModeStart(callback) {
  logger.info(
    'Monis Agent for Node.js starting in serverless mode -- skipping connection process.'
  )
