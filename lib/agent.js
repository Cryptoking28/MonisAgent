...
function Agent(options) {
  ...
  try {
    ...
    this.context = new Context(true); // FIXME: turn off debugging when ready to ship
...
Agent.prototype.scheduleRetry = function (error) {
  ...
  logger.error("An error occurred connecting to " +
               self.config.host + ":" +
               self.config.port + " - " +
               util.inspect(error));
...
/**
 * Examine shared context to find any curretn transaction.
 * Filter out inactive transactions.
 *
 * @returns {Transaction} The current transaction.
 */
Agent.prototype.getTransaction = function () {
  var stashed = this.context.transaction;
  if (stashed && stashed.value && stashed.value.isActive()) {
