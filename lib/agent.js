...
var MetricNormalizer = require('./metrics/normalizer')
var TxSegmentNormalizer = require('./metrics/normalizer/tx_segment')
var MetricMapper = require('./metrics/mapper')
var TraceAggregator = require('./transaction/trace/aggregator')
var hashes = require('./util/hashes')
var QueryTracer = require('./db/tracer')
var uninstrumented = require('./uninstrumented')
 ...
 */
Agent.prototype.adjustDistributedTraceStats = adjustDistributedTraceStats
function adjustDistributedTraceStats(target) {
...
Agent.prototype.incrementTransactionsSampled = function incrementTransactionsSampled() {
  ...
  if (this.transactionsSampled > this.sampledTarget) {
    ...
    // as well.
    this.adjustDistributedTraceStats(Math.pow(target, ratio) - Math.pow(target, 0.51))
  }
}
 ...
 */
Agent.prototype.shouldSampleDistributedTrace = shouldSampleDistributedTrace
function shouldSampleDistributedTrace(txn) {
...
Agent.prototype.reset = function reset() {
  ...
  if (this.config.debug.internal_metrics) {
    this._resetSupportability()
  }
}

Agent.prototype._resetSupportability = function _resetSupportability() {
...
Agent.prototype.harvest = function harvest(callback) {
  ...
  this.adjustDistributedTraceStats()
...
Agent.prototype._processErrorEvents = function _processErrorEvents() {
  ...
  if (this.config.error_collector.capture_events) {
    ...
    // clear the queue now (instead of at send time) so metrics match
    // what we actually send.
...
Agent.prototype._sendErrorEvents = function _sendErrorEvents(callback) {
  if (this.config.error_collector.capture_events && this._lastErrorEvents &&
    ...
    var rawEvents = this._lastErrorEvents[3]
    var run_id = agent.config.run_id
    ...
    agent.collector.errorEvents(payload, function onErrorEvents(err) {
      if (err && err.statusCode === 413 ) {
        ...
        agent.errors.mergeEvents(rawEvents)
      }
      callback(err)
    })
  } else {
