 ...
 * orchestrator for Monis Agent within instrumented applications.
 *
...
function Agent(config) {
  ...
  this.transactionsSampled = 0
  this.minSampledPriority = 0

  // This will become configurable by the server in the future.
  // There will also not be tied to the harvest time, and will have its
  // own period for sampling.
  this.sampledTarget = 10
...
Agent.prototype.start = function start(callback) {
  ...
  if (this.config.agent_enabled !== true) {
    logger.warn('The Monis Agent Node.js agent is disabled by its configuration. ' +
                'Not starting!')
  ...
  logger.info('Starting Monis Agent for Node.js connection process.')
...
/**
 * This adjusts the stats used to determine if a trace should be sampled
 * by the agent.
 *
 * @param {Number} target
 *  The target number of transactions to sample. Defaults to this.sampledTarget.
 */
Agent.prototype.adjustTransactionStats = function adjustTransactionStats(target) {
  if (this.transactionCreatedInHarvest) {
    target = target || this.sampledTarget
    // set the min to be 0 (take all the transactions) if we haven't hit
    // the target
    var samplingRatio = Math.min(target / this.transactionCreatedInHarvest, 1)
    this.minSampledPriority = 1 - samplingRatio
  }
}

/**
 * This method increments the transactionsSampled value and computes any
 * back off needed as the value increases.
 */
Agent.prototype.incrementTransactionsSampled = function incrementTransactionsSampled() {
  ++this.transactionsSampled
  if (this.transactionsSampled > this.sampledTarget) {
    var ratio = this.sampledTarget / this.transactionsSampled
    var target = this.transactionsSampled
    // This decay function will hit 0 at ~2x the target value due to the 0.51 constant.
    // This could probably be rewritten as target/max_samples, though the other agents
    // are using this constant.  In the interest of consistency we will use the constant
    // as well.
    this.adjustTransactionStats(Math.pow(target, ratio) - Math.pow(target, 0.51))
  }
}

/**
 * Determines if a transaction should be sampled based on the
 * transaction's priority and number of transactions have been sampled.
 *
 * @param {Transaction} txn The transaction to sample
 *
 * @returns {boolean} Whether the transaction was sampled
 */
Agent.prototype.shouldSampleTransaction = function shouldSampleTransaction(txn) {
  const sampled = txn.priority >= this.minSampledPriority
  if (sampled) {
    this.incrementTransactionsSampled()
  }
  return sampled
}
...
Agent.prototype.stop = function stop(callback) {
  ...
  if (this.collector.isConnected()) {
    this.collector.shutdown(function onShutdown(error) {
      if (error) {
        agent.setState('errored')
        logger.warn(error, 'Got error shutting down connection to Monis Agent:')
      } else {
        agent.setState('stopped')
        logger.info('Stopped Monis Agent for Node.js.')
      }
...
Agent.prototype.harvest = function harvest(callback) {
  ...
  this.recordSupportability(
    ...
    this.transactionCreatedInHarvest
  )

  this.adjustTransactionStats()
  ...
  if (!this.collector.isConnected()) {
    return process.nextTick(function onNextTick() {
      callback(new Error('Not connected to Monis Agent!'))
    })
  }
  runHarvestStep(0)
...
Agent.prototype._startHarvester = function _startHarvester(harvestSeconds) {
  ...
  function onError(error) {
    if (error) {
      logger.info(error, 'Error on submission to Monis Agent (data held for redelivery):')
    }
  }
...
/**
 * Assembles metrics for the application and sends them to the collector.
 *
...
Agent.prototype._sendMetrics = function _sendMetrics(callback) {
  ...
  if (this.collector.isConnected()) {
    if (this.errors.getTotalErrorCount() > 0) {
      ...
      count = this.errors.getOtherTransactionsErrorCount()
    ...
    this.collector.metricData(payload, function onMetricData(error, rules) {
      if (error) agent.metrics.merge(metrics, true)
    ...
    process.nextTick(function onNextTick() {
      callback(new Error('not connected to Monis Agent (metrics will be held)'))
    })
  }
}

/**
 * This function takes the custom events queue, gets stats on it for
 * metric purposes, then instantiates a new custom events queue. This is
...
Agent.prototype._processCustomEvents = function _processCustomEvents() {
  this.customEventsPool = this.customEvents
  ...
  if (this.config.custom_insights_events.enabled) {
    ...
    // a harvest cycle.
    this.customEventsPool = null
  }
  // Create a new queue now (instead of at send time) so metrics match
...
/**
 * This function takes the error events queue, gets stats on it for
 * metric purposes, then instantiates a new error events queue. This is
 * so the stats are consistent with what actually gets pushed by the later
 * call to _sendErrorEvents.
 */
Agent.prototype._processErrorEvents = function _processErrorEvents() {
  const events = this.errors.getEvents()
  ...
  // Create the metrics so they are at least set to 0
  const seen = this.metrics.getOrCreateMetric(NAMES.TRANSACTION_ERROR.SEEN)
  const sent = this.metrics.getOrCreateMetric(NAMES.TRANSACTION_ERROR.SENT)
  ...
  if (this.config.error_collector.capture_events) {
    // Record their values
    seen.incrementCallCount(this.errors.seen)
    sent.incrementCallCount(this.errors.length)

    // Log any warnings about dropping events
    const diff = this.errors.overflow
    if (diff) {
      logger.warn('Dropped %s error events out of %s.', diff, this.errors.seen)
    }

    // Clear the queue now (instead of at send time) so metrics match what we
    // actually send.
    this._lastErrorEvents[3] = this.errors.clearEvents()
...
Agent.prototype._sendErrors = function _sendErrors(callback) {
  ...
  if (this.config.collect_errors && this.config.error_collector.enabled) {
    if (!this.collector.isConnected()) {
      return process.nextTick(function onNextTick() {
        callback(new Error('not connected to Monis Agent (errors will be held)'))
      })
    ...
    this.collector.errorData(payload, function onErrorData(error) {
      if (error) agent.errors.mergeErrors(errors)
...
Agent.prototype._sendTrace = function _sendTrace(callback) {
  ...
  if (this.config.collect_traces && this.config.transaction_tracer.enabled) {
    if (!this.collector.isConnected()) {
      return process.nextTick(function onNextTick() {
        callback(new Error('not connected to Monis Agent (slow trace data will be held)'))
      })
    }

    this.traces.harvest(function onHarvest(error, traces, trace) {
      ...
      agent.collector.transactionSampleData(
        payload,
        function onTransactionSampleData(sampleError) {
          if (!sampleError) {
            agent.traces.reset(trace)
          }
...
Agent.prototype._sendEvents = function _sendEvents(callback) {
  if (this.config.transaction_events.enabled) {
    ...
    // clear events
    agent.events = new PriorityQueue(
      agent.config.transaction_events.max_samples_per_minute
    )
    ...
    agent.collector.analyticsEvents(payload, function onAnalyticsEvents(err) {
      if (err && err.statusCode === 413 ) {
        ...
        agent.events.setLimit(agent.config.transaction_events.max_samples_stored)

        agent.events.merge(events)
      } else {
...
Agent.prototype._sendCustomEvents = function _sendCustomEvents(callback) {
  ...
  if (this.config.custom_insights_events.enabled && this.customEventsPool.length > 0) {
    var agent = this
    var run_id = agent.config.run_id
    var events = this.customEventsPool.toArray()
    ...
    // send data to collector
    agent.collector.customEvents(payload, function onCustomEvents(err) {
      if (err && err.statusCode === 413 ) {
        ...
        agent.customEvents.merge(agent.customEventsPool)
      }
...
Agent.prototype._sendErrorEvents = function _sendErrorEvents(callback) {
  if (
    !this.config.error_collector.capture_events ||
    !this._lastErrorEvents ||
  ...
  const agent = this
  const eventsLimit = this._lastErrorEvents[0]
  const eventsSeen = this._lastErrorEvents[1]
  const events = this._lastErrorEvents[2]
  const eventQueue = this._lastErrorEvents[3]
  const run_id = agent.config.run_id
  ...
  // send data to collector
  agent.collector.errorEvents(payload, function onErrorEvents(err) {
    if (err && err.statusCode === 413 ) {
      logger.warn('request too large; event data dropped')
    } else if (err) {
      logger.warn('error events failed to send; re-sampling')
      agent.errors.mergeEvents(eventQueue)
    }
    callback(err)
  })
}
...
function _addIntrinsicAttrsFromTransaction(transaction) {
  ...
  if (this.config.cross_application_tracer.enabled) {
    if (this.config.feature_flag.distributed_tracing) {
      transaction._addDistributedTraceIntrinsics(intrinsicAttributes)
    } else if (
