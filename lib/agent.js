...
Agent.prototype.submitTransactionSampleData = function (encoded) {
  if (encoded) this.connection.sendTransactionTraces(encoded);
};
...
Agent.prototype.createTransaction = function () {
  var blank = new Transaction(this);

  callstack.annotateCaller(blank, 2);

  return blank;
};

/**
 * Crawl the stack to find the transaction onto which this context is bound.
 * Filter out dead transactions, although those should already automatically
 * remove themselves from the call stack. Managed by shimmer.
 *
 * @returns {Transaction} The current transaction.
 */
Agent.prototype.getTransaction = function () {
  var stashed = callstack.findAnnotation();
