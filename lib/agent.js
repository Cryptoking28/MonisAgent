...
function Agent(options) {
  ...
  this.metrics = new Metrics(this.config.apdex_t, this.mapper);
  if (this.config.debug.internal_metrics) {
    this.config.debug.supportability = new Metrics(this.config.apdex_t);
  }

  // transaction naming
  this.normalizer = new MetricNormalizer(this.config);
  this.normalizer.loadFromConfig();
...
Agent.prototype.start = function () {
  ...
  sampler.start(this);
  this.harvesterHandle = setInterval(this.harvest.bind(this), 60 * TO_MILLIS);
...
Agent.prototype.harvest = function () {
  if (this.connection && this.connection.isConnected()) {
    ...
    if (this.config.error_collector.enabled) this.submitErrorData();
    if (this.config.transaction_tracer.enabled) this.traces.harvest();
