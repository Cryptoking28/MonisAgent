...
Agent.prototype._apdexTChange = function _apdexTChange(apdexT) {
  ...
  // TODO: Ideally, we update all configuration items once upon
  // completed connect handshake, instead of per configuration item,
  // via metrics.reconfigure(config).
  this.metrics._apdexT = apdexT
  this.metrics._metrics.apdexT = apdexT
}
...
Agent.prototype._scheduleHarvester = function _scheduleHarvester(harvestSeconds) {
  ...
  this.harvesterHandle = setTimeout(function doHarvest() {
    ...
    agent.harvest(function harvestError(error) {
      if (error) {
        logger.warn(error, 'Error on submission to Monis Agent.')
      }
    })
  }, harvestDelay)
