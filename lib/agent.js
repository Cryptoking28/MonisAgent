...
Agent.prototype.getLinkingMetadata = function getLinkingMetadata() {
  let metric = this.metrics.getOrCreateMetric(
    NAMES.SUPPORTABILITY.API + '/getLinkingMetadata'
  )
  metric.incrementCallCount()

  let transaction = this.tracer.getTransaction()
  ...
  if (transaction) {
    linkingMetadata['trace.id'] = transaction.getTraceId()
    linkingMetadata['span.id'] = transaction.getSpanId()
  }
