...
function generateCPUMetricRecorder(agent) {
  var lastSampleTime
  return function recordCPUMetrics(userTimeInMicros, sysTimeInMicros) {
    var elapsedUptime
    if (!lastSampleTime) {
    ...
    var userTime = userTimeInMicros / MICROS
    var sysTime = sysTimeInMicros / MICROS
...
function sampleCpu(agent) {
  ...
  return function cpuSampler() {
    ...
    recordCPU(cpuSample.user, cpuSample.system)
  }
}
...
function sampleCpuNative(agent, nativeMetrics) {
  ...
  nativeMetrics.on('usage', function collectResourceUsage(usage) {
    recordCPU(usage.diff.ru_utime, usage.diff.ru_stime)
  })
  ...
  start: function start(agent) {
    ...
    if (!this.nativeMetrics) {
      if (metricFeatureFlag) {
        try {
          this.nativeMetrics = require('@monisagent/native-metrics')({
