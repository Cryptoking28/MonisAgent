...
function Agent() {
    ...
    this.connect = function(licenseKey, host, port) {
        setTimeout(function() {
            doConnect(licenseKey, host, port);
        }, this.applicationPort ? 0 : 1000);
    }

    function doConnect(licenseKey, host, port) {
        if (nrService) {
            return;
        }
        if (!host) {
        ...
        nrService = service.createMonisAgentService(self, licenseKey, host, port);
    ...
    */
    ...
    function loadInstrumentation() {
        ...
        files.forEach(function(name) {
            var fileName = "monisagent_agent/" + coreDir + "/" + name;
