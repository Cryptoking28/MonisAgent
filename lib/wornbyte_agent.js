...
function Agent(options) {
  ...
  function doConnect() {
    ...
    var connection = self.connection = self.options.connection || service.createMonisAgentService(self, self.config);
  ...
  if (!(options && options.connection)) {
    ...
    var generateShim = function (next, name) {
      ...
      if (currentTransaction && currentTransaction.isFinished()) {
        agent.clearTransaction(currentTransaction);
        currentTransaction = null;
      }

      return function () {
        agent.setTransaction(currentTransaction);
    ...
    // this isn't fully instrumented, but it does use a custom connection
    var customAgent = new Agent(options);
    customAgent.start();
    return customAgent;
  }
};
