...
function Agent() {
  ...
  function doConnect() {
    ...
    self.connection = service.createMonisAgentService(self, self.config);
    self.connection.on('connect', wrapCallback(self.config, self.config.onConnect));
    self.connection.on('connect', wrapCallback(self.metricNormalizer, self.metricNormalizer.parseMetricRules));
    self.connection.on('metricDataError', wrapCallback(self.statsEngine, self.statsEngine.mergeMetricData));
    self.connection.on('metricDataResponse', wrapCallback(self.statsEngine, self.statsEngine.parseMetricIds));
    self.connection.on('errorDataError', wrapCallback(self.errorService, self.errorService.onSendError));
