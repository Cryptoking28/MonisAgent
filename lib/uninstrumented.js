...
function logUninstrumented() {
  if (uninstrumented.length > 0) {
    var message =
      'The monisagent module must be the first module required.\n' +
      'The following modules were required before monisagent and are not being ' +
      'instrumented:'
...
function moduleNameFromFilename(filename) {
  ...
  var paths = parsed.dir.split(path.sep)

  if (name !== 'index' && name !== 'application') { // TODO: workaround for koa
    return paths[paths.lastIndexOf('node_modules') + 1] === name ? name : null
  }

  for (var i = paths.length - 1; i >= 0; i--) {
    if (paths[i] !== 'lib') {
      return paths[i - 1] === 'node_modules' ? paths[i] : null
    }
  }
}
