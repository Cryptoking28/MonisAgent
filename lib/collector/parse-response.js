...
module.exports = function parse(name, response, callback) {
  ...
  return function parser(inError, body) {
    ...
    if (body) {
      try {
        ...
        if (returned && returned.messages) {
          returned.messages.forEach(function (element) { logger.info(element.message); });
        }
        ...
        if (exception) {
          if (exception.message) {
            ...
            errors.push(new Error('Monis Agent internal error'));
          }
    ...
    process.nextTick(function () { callback(error, returned, json); });
  };
};
