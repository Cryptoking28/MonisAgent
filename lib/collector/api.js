...
const responseCodesMap = require('./response-codes')
...
CollectorAPI.prototype._runLifecycle = function _runLifecycle(method, body, callback) {
  ...
  function standardHandler(error, returned) {
    ...
    const responseCodeHandler = responseCodesMap[error.statusCode]
    if (responseCodeHandler) {
      return responseCodeHandler(error, returned, method, callback, api)
    }
