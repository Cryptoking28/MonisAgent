...
function CollectorAPI(agent) {
  this._agent = agent
  this._reqHeadersMap = Object.create(null)
...
CollectorAPI.prototype._login = function _login(callback) {
  ...
  methods.redirect.invoke(payload, this._reqHeadersMap, onPreConnect)
...
CollectorAPI.prototype._connect = function _connect(env, callback) {
  ...
  methods.handshake.invoke(env, this._reqHeadersMap, onConnect)
  ...
  function onConnect(error, config, body) {
    ...
    if (config.request_headers_map) {
      // Store request headers for future collector requests
      CollectorAPI._reqHeadersMap = config.request_headers_map
    }
...
CollectorAPI.prototype.errorEvents = function errorEvents(events, callback) {
  if (!events) throw new TypeError('must pass errors to send')
  if (!callback) throw new TypeError('callback is required')
...
CollectorAPI.prototype.spanEvents = function spanEvents(events, callback) {
  if (!events) throw new TypeError('must pass spans to send')
  if (!callback) throw new TypeError('callback is required')
...
CollectorAPI.prototype.transactionSampleData =
  function transactionSampleData(trace, callback) {
    if (!trace) {
      throw new TypeError("must pass slow trace data to send")
    }
...
CollectorAPI.prototype.shutdown = function shutdown(callback) {
  ...
  function onShutdown(error, returned, body) {
    if (error) {
...
CollectorAPI.prototype._runLifecycle = function _runLifecycle(method, body, callback) {
  ...
  function standardHandler(error, returned, json) {
    ...
    if (error.statusCode === HTTP_REQUEST_TOO_LARGE) {
      logger.error(
        error,
        "This call of %s sent Monis Agent too much data; discarding (%s):",
        method.name,
      ...
      logger.error(
        error,
        "The Monis Agent collector couldn't deserialize data; discarding for %s (%s):",
        method.name,
      ...
      logger.debug(
        error,
        "Monis Agent is experiencing a spot of bother; please hold on (%s):",
        HTTP_LOL_COLLECTOR
      )
      ...
      logger.error(
        error,
        "Monis Agent's servers encountered a severe internal error on %s (%s):",
        method.name,
      ...
      logger.error(
        error,
        "Your Monis Agent license key appears to be invalid. Please double-check it:"
      )
      ...
      logger.error(
        error,
        "Monis Agent ran into a weird problem with %s. Let support@monisagent.com know:",
        method.name
      )
      return callback(null, returned, json)
      ...
      logger.info(
        error,
        "The Monis Agent collector requested a connection restart on %s:",
        method.name
      )
      ...
      logger.error(error, "The Monis Agent collector is shutting down this agent:")
      ...
      logger.info(
        error,
        "The Monis Agent server for your account is currently undergoing maintenance. " +
      ...
      logger.warn(
        error,
        "Calling %s on Monis Agent failed due to a runtime error. " +
    ...
    logger.error(
      error,
      "Calling %s on Monis Agent failed unexpectedly. " +
        "Data will be held until it can be submitted:",
      method.name
    )
    return callback(error, returned, json)
