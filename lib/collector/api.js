...
/**
 * Send already-formatted transaction tracedata by calling transaction_sample_data. For
 ...
 *
 * 1. The agent run ID.
 * 2. An array of one or more traces.
 *
 * @param {Array}    traces   The encoded traces list.
 * @param {Function} callback The continuation / error handler.
 */
CollectorAPI.prototype.transaction_sample_data = function traceData(traces, callback) {
  if (!callback) {
    throw new TypeError('callback is required')
  }
  if (!traces) {
...
CollectorAPI.prototype._sendData = function _sendData(method, data, callback) {
  this._runLifecycle(method, data, (error, response) => {
    ...
    if (response.agentRun === AGENT_RUN_BEHAVIOR.SHUTDOWN) {
      ...
      callback({retainData: response.retainData})
      ...
      this.restart(function afterRestart(restartError) {
        if (restartError) {
