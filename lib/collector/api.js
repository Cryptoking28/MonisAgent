...
/**
 * @param {Agent} agent Monis Agent agent
 */
...
CollectorAPI.prototype._retry = function _retry(ctx, error, response) {
  ...
  if (response.status === 410 || response.agentRun === AGENT_RUN_BEHAVIOR.SHUTDOWN) {
    ...
    logger.warn(
      ...
      'Your proxy server appears to be configured to accept connections over http. ' +
        'When setting `proxy_host` and `proxy_port` Monis Agent attempts to connect over ' +
    ...
    // status code.
    let msg = 'Unexpected error communicating with Monis Agent backend.'
    if (response.status) {
      msg = `Received error status code from Monis Agent backend: ${response.status}.`
    }
  ...
  if (backoff.warn) {
    logger.warn('No connection has been established to Monis Agent after %d attempts.', ctx.attempts)
  }
...
CollectorAPI.prototype.shutdown = function shutdown(callback) {
  ...
  function onShutdown(error, response) {
    ...
    logger.info('Disconnected from Monis Agent; clearing run ID %s.', agent.config.run_id)
...
function _handleResponseCode(response, endpoint, cb) {
  ...
  if (SUCCESS.has(code)) {
    ...
    // Monis Agent doesn't like us and we shouldn't try to talk to them any more.
    ...
    // We're not sure what Monis Agent is trying to tell us. Let's get rid of our
