...
CollectorAPI.prototype.connect = function connect(callback) {
  ...
  function retry(error, response, body) {
    ...
    if (backoff.warn) {
      logger.warn(
        "No connection has been established to Monis Agent after %s attempts.",
        attempts
      );
    }

    logger.debug(
      "Failed attempting to connect to Monis Agent, waiting %ss to retry.",
      backoff.interval
    );
...
CollectorAPI.prototype.errorData = function errorData(errors, callback) {
  ...
  var payload = [this._agent.config.run_id, errors];
  this._runLifecycle(this._methods.errors, payload, callback);
};
...
CollectorAPI.prototype.metricData = function metricData(metrics, callback) {
  ...
  var beginMillis = metrics.started * FROM_MILLIS
    ...
    , payload     = [this._agent.config.run_id, beginMillis, endMillis, metrics]
    ;

  this._runLifecycle(this._methods.metrics, payload, callback);
};
...
CollectorAPI.prototype.transactionSampleData =
  ...
  var payload = [this._agent.config.run_id, traces];
  this._runLifecycle(this._methods.traces, payload, callback);
};
...
CollectorAPI.prototype.sqlTraceData = function sqlTraceData(sqls, callback) {
  ...
  var payload = [this._agent.config.run_id, sqls];
  this._runLifecycle(this._methods.sqls, payload, callback);
};
...
CollectorAPI.prototype._runLifecycle = function _runLifecycle(method, body, callback) {
  ...
  function standardHandler(error, returned, json) {
    ...
    if (error.statusCode === HTTP_REQUEST_TOO_LARGE) {
      logger.error(
        error,
        "This agent sent the Monis Agent collector too much data; discarding for %s (%s):",
        method.name,
