...
CollectorAPI.prototype.connect = function connect(callback) {
  ...
  function retry(error, response) {
    ...
    if (response.status === 410 || response.shutdownAgent) {
      // TODO: still don't have a mechanism for shutting down without an error
...
CollectorAPI.prototype._login = function _login(callback) {
  ...
  function onPreConnect(error, response) {
    ...
    agent.config.applyLasp(agent, policies, function onApply(err, obj) {
      if (err || obj.shutdownAgent) {
        return callback(err, obj)
      }

      self._getFacts(obj.payload, callback)
    })
  }
}
...
CollectorAPI.prototype.shutdown = function shutdown(callback) {
  ...
  function onShutdown(error, response) {
    ...
    } else if (SUCCESS.has(response.status)) { // TODO: potentially disregard status code
