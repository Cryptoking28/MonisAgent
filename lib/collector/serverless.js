...
const defaultPipePath = path.resolve('/tmp', 'monisagent-telemetry')
...
class ServerlessCollector {
  ...
  constructor(agent, nrPipePath) {
    ...
    this.payload = {}
    this.NEWRELIC_PIPE_PATH = nrPipePath || process.env.NEWRELIC_PIPE_PATH || defaultPipePath
    this.shouldUsePipe = fs.existsSync(this.NEWRELIC_PIPE_PATH)
  }
  ...
  flushToPipe(payload) {
    try {
      fs.writeFileSync(this.NEWRELIC_PIPE_PATH, payload)
    } catch (e) {
  ...
  _doFlush(payload, sync = false) {
    ...
    const didUsePipe = this.shouldUsePipe && this.flushToPipe(serializedPayload)
