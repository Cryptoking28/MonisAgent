...
var USER_AGENT_FORMAT = 'MonisAgent-NodeAgent/%s (nodejs %s %s-%s)'
 ...
 */
RemoteMethod.prototype.invoke = function call(payload, nrHeaders, callback) {
  if (typeof nrHeaders === 'function') {
    callback = nrHeaders
    nrHeaders = Object.create(null)
  }
 ...
 * @param {number} options.nrHeaders  - NR request headers from the connect response.
...
RemoteMethod.prototype._headers = function _headers(options) {
  ...
  if (options.nrHeaders) {
    headers = Object.assign(headers, options.nrHeaders)
  }
