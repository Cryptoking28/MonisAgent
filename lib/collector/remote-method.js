...
const { SUPPORTABILITY, DATA_USAGE } = require('../metrics/names')

function getMetricName(name) {
  return `${SUPPORTABILITY.NODEJS}/Collector/${name}/${DATA_USAGE.SUFFIX}`
}
...
const USER_AGENT_FORMAT = 'MonisAgent-NodeAgent/%s (nodejs %s %s-%s)'
const ENCODING_HEADER = 'CONTENT-ENCODING'
const DEFAULT_ENCODING = 'identity'

function RemoteMethod(name, config, endpoint, metrics) {
  if (!name) {
...
RemoteMethod.prototype.invoke = function invoke(payload, nrHeaders, callback) {
  const reportDataUsage = (sent, received) => {
    if (this._metrics) {
      ...
      this._metrics.measureBytes(DATA_USAGE.COLLECTOR, ...measurement)
      this._metrics.measureBytes(getMetricName(this.name), ...measurement)
    }
  }
    ...
    function onSerialize(err, serialized) {
      ...
      this._post(
        ...
        function onResponse(err, res) {
          reportDataUsage(serialized, res && res.payload)
          callback(err, res)
        }.bind(this)
      )
    }.bind(this)
    ...
    if (this._config.certificates && this._config.certificates.length > 0) {
      ...
      if (this._config.feature_flag.certificate_bundle) {
        ...
        const certWarningMessage = [
          'Deprecation Warning: The certificate bundle included by Monis Agent will be ',
