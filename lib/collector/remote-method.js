  ...
  function parse(error, body) {
    ...
    if (json.exception) {
      ...
      if (exception.message) {
        converted = new Error(exception.message);
      }
      else {
        converted = new Error('Monis Agent internal error');
      }
      converted.statusCode = 200;
      if (exception.error_type) converted.class = exception.error_type;

      return callback(converted, returned, json);
    }

    // Ensure that some data did make it through if there's no exception.
    if (returned === undefined || returned === null) {
      return callback(
        new Error(util.format("No data found in response to %s.", sender.name)),
        returned,
        json
      );
    }

    // raw json is useful for tests
