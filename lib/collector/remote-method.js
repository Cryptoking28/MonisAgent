    ...
    function onSerialize(err, serialized) {
      ...
      this._post(
        ...
        function onResponse(err, res) {
          ...
          if (this._metrics) {
            const payloadByteLength = byteLength(serialized)
            const responseByteLength = res.payload ? byteLength(JSON.stringify(res.payload)) : 0
            this._metrics.measureBytes(
              `${NAMES.SUPPORTABILITY.NODEJS}/Collector/Output/Bytes`,
              payloadByteLength,
              responseByteLength,
              true
            )
            this._metrics.measureBytes(
              `${NAMES.SUPPORTABILITY.NODEJS}/Collector/${this.name}/Output/Bytes`,
              payloadByteLength,
              responseByteLength,
              true
            )
