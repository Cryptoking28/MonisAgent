...
module.exports = {
  ...
  400: (err, returned, method, cb) => {
    logger.error(
      err,
      'Bad request to Monis Agent %s method; discarding data',
      method.name
    )
  ...
  407: (err, returned, method, cb) => {
    logger.error(
      err,
      'Monis Agent returned a response that requires proxy authentication. ' +
  ...
  408: (err, returned, method, cb) => {
    logger.warn(
      'Request to Monis Agent has timed out. Review your network settings. ' +
      'If this regularly occurs, reach out to Monis Agent support.'
    )
  ...
  410: (err, returned, method, cb, api) => {
    logger.error(err, `The Monis Agent collector is shutting down this agent:`)
  ...
  411: (err, returned, method, cb) => {
    logger.warn(
      'Monis Agent %s call was missing required Content-Length header.',
      method.name
    )
    cb(null, CollectorResponse.error(returned))
  },
  ...
  414: (err, returned, method, cb) => {
    logger.warn('Monis Agent %s request URI too long; discarding data', method.name)
    cb(null, CollectorResponse.success(returned))
  },
  /**
  ...
  417: (err, returned, method, cb) => {
    logger.warn('Monis Agent %s expectation failed; discarding data', method.name)
    cb(null, CollectorResponse.success(returned))
  },
  /**
  ...
  429: (err, returned, method, cb) => {
    logger.warn(
      'Monis Agent %s endpoint experiencing too many requests; reattempting',
      method.name
    )
  ...
  431: (err, returned, method, cb) => {
    logger.warn('Monis Agent %s request headers too large; discarding data', method.name)
    cb(null, CollectorResponse.success(returned))
  },
  /**
