 ...
 *
 * connect() -> onRedirect() -> onHandshakeResponse() --> emit 'connect'
 */
CollectorConnection.prototype.connect = function () {
  this.once('connectResponse', this.onRedirect.bind(this));
 ...
 *
 * FIXME: should emit error to restart the connection process.
 */
CollectorConnection.prototype.onHandshakeResponse = function (response) {
  if (response.agent_run_id) {
    ...
    logger.error("Connection attempt unsuccessful. Check your configuration!");
  }
};
 ...
 * the collector proxy.
 */
CollectorConnection.prototype.onRedirect = function (redirectHost) {
  if (redirectHost) {
