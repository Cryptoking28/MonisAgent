...
CollectorConnection.prototype.destroy = function () {
  logger.info("Shutting down Monis Agent connection with run ID %s.", this.agentRunId);
...
CollectorConnection.prototype.sendShutdown = function () {
  if (this.isConnected()) {
    ...
    logger.info("Not connected to Monis Agent; not attempting to shut down connection.");
  }
};
