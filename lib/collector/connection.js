...
function hasError(error, value) {
  return error &&
         error.error_type &&
         error.error_type === 'MonisAgent::Agent::' + value;
}
...
CollectorConnection.prototype.destroy = function () {
  logger.info("Destroyed Monis Agent connection for agent run ID %s.",
              this.agentRunId);
  this.agentRunId = null;
};
...
CollectorConnection.prototype.sendTracedErrors = function (errors) {
  ...
  if (!this.isConnected()) {
    return logger.warn("Not connected to Monis Agent. Not sending noticed errors.");
  }
...
CollectorConnection.prototype.sendMetricData = function (metrics) {
  ...
  if (!this.isConnected()) {
    return logger.warn("Not connected to Monis Agent. Not sending metric data.");
  }

  var beginMillis = metrics.started / 1000
    , endMillis   = Date.now() / 1000
    , data        = [this.agentRunId, beginMillis, endMillis, metrics]
    ;
...
CollectorConnection.prototype.sendTransactionTraces = function (traces) {
  ...
  if (!this.isConnected()) {
    return logger.warn("Not connected to Monis Agent. Not sending slow trace.");
  }
...
CollectorConnection.prototype.sendSQLTraces = function (sqls) {
  ...
  if (!this.isConnected()) {
    return logger.warn("Not connected to Monis Agent. Not sending slow SQL.");
  }
...
CollectorConnection.prototype.sendShutdown = function () {
  if (!this.isConnected()) {
    return logger.info("Not connected to Monis Agent. " +
...
CollectorConnection.prototype.onShutdown = function () {
  logger.info("Connection to Monis Agent terminated.");
  this.destroy();
  this.emit('close');
};
...
CollectorConnection.prototype.onNRException = function (method, error) {
  if (isRestart(error) && this.isConnected()) {
    logger.debug("Monis Agent wants the agent to reconnect.");
    this.agent.emit('restart');
  }
  else if (isDisconnect(error) && this.isConnected()) {
    logger.debug("Monis Agent has instructed this agent to shut down: %s",
                 error.message);
    ...
    logger.error("The Monis Agent server for your account is currently " +
                 "unavailable. Data will be held until it can be submitted: %s",
                 error.message);
  }
  else if (isRuntime(error)) {
    logger.error("Monis Agent's servers are currently unavailable due to a " +
