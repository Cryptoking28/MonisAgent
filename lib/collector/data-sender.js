...
var PROTOCOL_VERSION        = 9
  ...
  // https://hudson.monisagent.com/job/collector-master/javadoc/com/nr/servlet/AgentListener.html
...
DataSender.prototype.onError = function (message, error) {
  ...
  if (error.error_type !== 'MonisAgent::Agent::ForceRestartException') {
    logger.debug(error,
...
DataSender.prototype.onCollectorResponse = function (message, response) {
  ...
  if (response.statusCode < 200 || response.statusCode >= 300) {
    logger.error("Got %s as a response code from the error collector.",
                 response.statusCode);
    return this.emit('error',
                     message,
                     new Error("Got an HTTP error status of " +
                               response.statusCode));
  }
