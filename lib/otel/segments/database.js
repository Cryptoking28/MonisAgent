...
module.exports = function createDbSegment(agent, otelSpan) {
  ...
  const system = dbAttr({ key: 'system', span: otelSpan })
...
function parseStatement(config, otelSpan, system) {
  let table = dbAttr({ key: 'table', span: otelSpan })
  let operation = dbAttr({ key: 'operation', span: otelSpan })
  let statement = dbAttr({ key: 'query', span: otelSpan })
