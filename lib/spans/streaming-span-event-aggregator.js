...
class StreamingSpanEventAggregator extends EventAggregator {
  ...
  addSegment(segment, parentId, isRoot) {
    // TODO: idea... inject attribute collection type?
    ...
    for (let [key, value] of Object.entries(collectorSpan.intrinsics)) {
      ...
      intrinsics[key] = this.mapAttributeToProtoSpec(value)
    }
  ...
  mapAttributeToProtoSpec(value) {
    const stringType = 'string_value'
    const boolType = 'bool_value'
    const intType = 'int_value'
    const doubleType = 'double_value'
    ...
    switch (valueType) {
      case 'string': {
        protoTypeString = stringType
      }
      case 'boolean': {
        protoTypeString = boolType
      }
      case 'number': {
        const isInteger = Number.isInteger(value)
        protoTypeString = isInteger ? intType : doubleType
      }
      default: {
        // TODO: log in some safe manner?
      }
    }
