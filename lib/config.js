...
var stringify = require('./util/json-stringify-safer')
var logger
...
Config.prototype._applyHighSecurity = function _applyHighSecurity() {
  var config = this
  HIGH_SECURITY_KEYS.forEach(function checkKeys(key) {
    var hsValue = HIGH_SECURITY_SETTINGS[key]
    if (config[key] !== hsValue) {
      logger.warn('High Security Mode: %s was set to %s, coercing to %s',
                  key, config[key], hsValue)
      config[key] = hsValue
      config.emit(key, hsValue)
    }
  })
}
...
Config.prototype.publicSettings = function publicSettings() {
  ...
  settings = JSON.parse(stringify(settings))
