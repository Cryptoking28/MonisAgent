...
function Config(config) {
  ...
  // high security mode should only be updated by the server
  this.high_security = false;

  this.browser_monitoring.loader = 'rum';
  this.browser_monitoring.loader_version = '';

  // TODO: this will move to config.default.js after release
  this.analytics_events = {
    enabled: true, // enabled, but still hidden behind a feature flag
    max_samples_per_minute: 1000, // will drop events after reaching this limit
    max_samples_stored: 2000,     // not used yet
  };
...
Config.prototype._fromServer = function (params, key) {
  switch (key) {
    ...
    case 'high_security':
      logger.info("High security mode enabled by server");
    ...
    case 'analytics_events.max_samples_stored':
      this._updateNestedIfChanged(
        params,
        this.analytics_events,
        key,
    ...
    case 'analytics_events.max_samples_per_minute':
      this._updateNestedIfChanged(
        params,
        this.analytics_events,
        key,
...
Config.prototype.logUnknown = function (json, key) {
  var value = json[key];
  logger.debug(
    "Monis Agent sent unknown configuration parameter %s with value %j.",
    key,
    value
  );
};
...
Config.prototype.validateFlags = function () {
  Object.keys(this.feature_flag).forEach(function (key) {
