...
Config.prototype.onConnect = function (params) {
  ...
  if (!this.ignore_server_configuration) {
    ...
    this.logDisabled(params, 'apdex_t');
    this.logDisabled(params, 'data_report_period');
    this.logDisabled(params, 'capture_params');
    this.logDisabled(params, 'ignored_params');
    this.logDisabled(params, 'transaction_tracer.enabled');
    this.logDisabled(params, 'transaction_tracer.transaction_threshold');
    this.logDisabled(params, 'error_collector.enabled');

    this.logIgnored(params, 'sampling_rate');
    this.logIgnored(params, 'cross_process_id');
    this.logIgnored(params, 'cross_application_tracing');
    this.logIgnored(params, 'encoding_key');
    this.logIgnored(params, 'trusted_account_ids');
    this.logIgnored(params, 'high_security');
    this.logIgnored(params, 'ssl');
    this.logIgnored(params, 'transaction_tracer.record_sql');
    this.logIgnored(params, 'slow_sql.enabled');
    this.logIgnored(params, 'rum.load_episodes_file');
  }
...
Config.prototype._updateNestedIfChanged = function (remote, local, remoteKey, localKey) {
  ...
  if (value !== null && value !== undefined && local[localKey] !== value) {
    ...
    logger.info("Configuration of %s was changed to %s by Monis Agent.", remoteKey, value);
  }
};
...
/**
 * We would normally do something with this parameter, but server-side
...
/**
 * The agent doesn't use this parameter and server-side configuration is
 * ignored anyway. Worded differently to help support.
 *
 * @param {object} json Config blob sent by collector.
 * @param {string} key  Value the agent can't set.
 */
Config.prototype.logIgnored = function (json, key) {
  var value = json[key];
  if (value !== null && value !== undefined) {
    logger.debug(
      "Configuration of %s is currently ignored. " +
      "(Server sent value of %j.)",
      key,
      value
    );
...
function initialize(bootstrapLogger, c) {
  ...
  if (c && c.config) {
    ...
    if (isTruthular(process.env.NEW_RELIC_NO_CONFIG_FILE)) {
      ...
      if (envOnly.monisagent_home) delete envOnly.monisagent_home;
      return envOnly;
    }
    else {
      var filepath;
