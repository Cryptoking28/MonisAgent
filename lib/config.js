...
var util         = require('util')
  ...
  , label_parser = require('./util/label-parser')
...
// pairs are semicolon-delimited, key and value are colon-delimited
var KEY_VALUE_VARS = [
  "NEW_RELIC_LABELS"
]
...
function fromKeyValueList(setting) {
  var results = label_parser.fromString(setting)

  for(var i = 0, l = results.warnings.length; i < l; ++i) {
    logger.warn(results.warnings[i])
  }
...
function transformLabels(labels) {
  ...
  if (!labels) {
    ...
    results = fromKeyValueList(labels)
  } else if (labels) {
    results = label_parser.fromMap(labels)
  }

  results.warnings.forEach(function(messaage) {
...
function Config(config) {
  ...
  // transfrom labels before loading settings from enviroment
  this.labels = transformLabels(this.labels)
  ...
  Object.keys(metadata).forEach(function cb_forEach(value) {
    ...
    if (typeof node === 'string') {
      ...
      if (setting) {
        ...
        } else if (KEY_VALUE_VARS.indexOf(node) > -1) {
          data[value] = fromKeyValueList(setting)
        } else {
          data[value] = setting
        }
      }
