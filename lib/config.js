...
function fromObjectList(setting) {
  try {
    ...
    console.error("Monis Agent configurator could not deserialize object list:");
 ...
 *
 * @param {MetricNormalizer} normalizer Metric renamer from agent.
 * @param {Logger}           logger     Can't require logger because it
 *                                      depends on config, so must inject.
 */
Config.prototype.loadRules = function (normalizer, logger) {
  if (this.rules && this.rules.name && this.rules.name.length > 0) {
    this.rules.name.forEach(function (rule) {
      ...
      normalizer.addSimple(rule.pattern, rule.name);
    });
  }

  if (this.rules && this.rules.ignore && this.rules.ignore.length > 0) {
    this.rules.ignore.forEach(function (pattern) {
      normalizer.addSimple(pattern);
    });
  }
};
