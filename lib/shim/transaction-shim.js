...
const DISTRIBUTED_TRACE_HEADER = 'monisagent'
const HTTP_CAT_ID_HEADER = 'X-MonisAgent-Id'
const MQ_CAT_ID_HEADER = 'MonisAgentID'
const MATCH_CAT_ID_HEADER = new RegExp(
  '^(?:' + HTTP_CAT_ID_HEADER + '|' + MQ_CAT_ID_HEADER + ')$',
  'i'
)
const HTTP_CAT_TRANSACTION_HEADER = 'X-MonisAgent-Transaction'
const MQ_CAT_TRANSACTION_HEADER = 'MonisAgentTransaction'
const MATCH_CAT_TRANSACTION_HEADER = new RegExp(
  '^(?:' + HTTP_CAT_TRANSACTION_HEADER + '|' + MQ_CAT_TRANSACTION_HEADER + ')$',
  'i'
)
const HTTP_CAT_APP_DATA_HEADER = 'X-MonisAgent-App-Data'
const MQ_CAT_APP_DATA_HEADER = 'MonisAgentAppData'
...
function handleCATHeaders(headers, segment, transportType) {
  ...
  if (this.agent.config.distributed_tracing.enabled) {
    // TODO: handle based on format? maye rename overall function?
    // TODO: where is this function used? Seems to be by message-shim... 
    // do we even add DT to message stuff?
    // TODO: if not... maybe kill off DT from in here.
    // if so... rename and also remove check for
    // CAT at top. DT does not require CAT.
    // TODO: also only handles one casing... is it lowercased by default?

    // Always add trace context headers if only to pass them through our system.
    if (this.agent.config.feature_flag.dt_format_w3c) {
      if (!headers[TRACE_CONTEXT_PARENT_HEADER]) {
        const traceContext = tx.createTraceContextHeaders()
        headers[TRACE_CONTEXT_PARENT_HEADER] = traceContext.parent
      }
    }
