...
const DISTRIBUTED_TRACE_HEADER = 'X-MonisAgent-Trace'
...
}).reduce(function reduceToTypeMap(o, t) {
  o[t] = true
  return o
}, Object.create(null))
...
function handleCATHeaders(headers, segment) {
  ...
  if (this.agent.config.feature_flag.distributed_tracing) {
    // TODO: Process the incoming headers and stash the relevant data on
    // the transaction.
  } else {
...
function insertCATRequestHeaders(headers, useAlternateHeaderNames) {
  ...
  if (this.agent.config.feature_flag.distributed_tracing) {
    ...
    txData = tx.createDistributedTracePayload()
  } else {
    ...
    txData = JSON.stringify([tx.id, false, tx.tripId || tx.id, pathHash])
  }
