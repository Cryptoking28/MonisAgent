...
var HTTP_CAT_ID_HEADER = 'X-MonisAgent-Id'
var MQ_CAT_ID_HEADER = 'MonisAgentId'
var MATCH_CAT_ID_HEADER = new RegExp(
  '^(?:' + HTTP_CAT_ID_HEADER + '|' + MQ_CAT_ID_HEADER + ')$',
  'i'
)
var HTTP_CAT_TRANSACTION_HEADER = 'X-MonisAgent-Transaction'
var MQ_CAT_TRANSACTION_HEADER = 'MonisAgentTransaction'
var MATCH_CAT_TRANSACTION_HEADER = new RegExp(
  '^(?:' + HTTP_CAT_TRANSACTION_HEADER + '|' + MQ_CAT_TRANSACTION_HEADER + ')$',
  'i'
)
var HTTP_CAT_APP_DATA_HEADER = 'X-MonisAgent-App-Data'
var MQ_CAT_APP_DATA_HEADER = 'MonisAgentAppData'
var MATCH_CAT_APP_DATA_HEADER = new RegExp(
  '^(?:' + HTTP_CAT_APP_DATA_HEADER + '|' + MQ_CAT_APP_DATA_HEADER + ')$',
  'i'
)

var MATCH_CONTENT_LENGTH_HEADER = /^Content-Length$/i

// TODO: Turn this into an actual `Set` when deprecating Node <0.12
var TRANSACTION_TYPES_SET = Object.keys(Transaction.TYPES).map(function typesToValues(t) {
  return Transaction.TYPES[t]
})
...
TransactionShim.prototype.insertCATHeaders = insertCATHeaders
...
function bindCreateTransaction(nodule, property, spec) {
  ...
  if (TRANSACTION_TYPES_SET.indexOf(spec.type) === -1) {
    this.logger.error(
      {stack: (new Error()).stack},
      'Invalid spec type "%s", must be one of %j.',
      spec.type, TRANSACTION_TYPES_SET
    )
    return nodule
  }
...
function handleCATHeaders(headers) {
  ...
  if (catId && transactionData) {
    ...
    if (tx.incomingCatId) {
      this.logger.trace('Got inbound CAT headers in transaction %s', tx.id)
    }
  }
...
function insertCATHeaders(headers, useAlternateHeaderNames) {
  // Is CAT enabled?
  if (!this.agent.config.cross_application_tracer.enabled) {
  ...
  var transHeader = HTTP_CAT_TRANSACTION_HEADER
  ...
  try {
    var txData = JSON.stringify([tx.id, false, tx.tripId || tx.id, pathHash])
    txData = hashes.obfuscateNameUsingKey(txData, this.agent.config.encoding_key)
...
function insertCATReplyHeader(headers, useAlternateHeaderNames) {
  ...
  var contentLength = -1
  for (var key in headers) {
    if (MATCH_CONTENT_LENGTH_HEADER.test(key)) {
  ...
  var appData = null
  var txName = tx.name || tx.nameState.getName() || ''
  try {
    if (txName) {
      txName = metrics.WEB.RESPONSE_TIME + '/' + txName
    }
...
function _handleCATReplyHeader(shim, segment, appData) {
  ...
  shim.logger.trace(
    'Got inbound response CAT headers for transaction %s',
