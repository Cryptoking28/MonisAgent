 ...
 *
 * - `handleCATHeaders(headers [, segment])`
 *
 * @memberof TransactionShim.prototype
 *
 ...
 */
function handleCATHeaders(headers, segment) {
  // Is CAT enabled?
...
function insertCATRequestHeaders(headers, useAlternateHeaderNames) {
  ...
  try {
    var txData = stringify([tx.id, false, tx.tripId || tx.id, pathHash])
...
function insertCATReplyHeader(headers, useAlternateHeaderNames) {
  ...
  try {
    appData = hashes.obfuscateNameUsingKey(stringify([
