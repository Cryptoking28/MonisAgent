...
function insertCATRequestHeaders(headers, useAlternateHeaderNames) {
  const isCrossAppTracingEnabled = this.agent.config.cross_application_tracer.enabled
  const isDistributedTracingEnabled = this.agent.config.distributed_tracing.enabled

  if (!isDistributedTracingEnabled && !isCrossAppTracingEnabled) {
    this.logger.trace(
