...
function _linkExceptionToSegment(exception) {
  ...
  segment.addAttribute('error.message', error.message)
  segment.addAttribute('error.name', error.name)
...
function _addException(exception) {
  const linkedException = this._linkExceptionToSegment(exception)
  this.exceptions.push(linkedException)
}
...
function _addUserError(exception) {
  const linkedException = this._linkExceptionToSegment(exception)
  this.userErrors.push(linkedException)
}
...
function insertDistributedTraceHeaders(headers) {
  ...
  try {
    const monisagentFormatData = this.createDistributedTracePayload().httpSafe()
