...
function acceptDistributedTraceHeaders(transportType, headers) {
  ...
  if (this.agent.config.feature_flag.dt_format_w3c) {
    ...
    traceparent = headers[TRACE_CONTEXT_PARENT_HEADER]
  }
...
function acceptTraceContextPayload(traceparent, tracestate, transport) {
  if (this.agent.config.feature_flag.dt_format_w3c !== true) {
    return
  }
  ...
  if (traceContext.acceptedTraceparent) {
    ...
    if (traceContext.acceptedTracestate) {
      ...
      this.agent.recordSupportability('Supportability/TraceContext/Accept/Success')
    }
  }
}
