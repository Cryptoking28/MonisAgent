...
const NEWRELIC_TRACE_HEADER = 'monisagent'

const MULTIPLE_INSERT_MESSAGE =
  'insertDistributedTraceHeaders called on headers object that already contains ' +
  "distributed trace data. These may be overwritten. traceparent? '%s', monisagent? '%s'."
 ...
 * W3C TraceContext format is preferred over the MonisAgent DT format.
 * MonisAgent DT format will be used if no `traceparent` header is found.
 *
...
function acceptDistributedTraceHeaders(transportType, headers) {
  ...
  if (traceparent) {
    ...
    logger.trace('Accepting monisagent DT payload for transaction %s', this.id)
    // assumes header keys already lowercase
...
function insertDistributedTraceHeaders(headers) {
  ...
  if (this.agent.config.distributed_tracing.exclude_monisagent_header) {
    logger.trace('Excluding monisagent header due to exclude_monisagent_header: true')
    return
  }

  try {
    const monisagentFormatData = this._createDistributedTracePayload().httpSafe()
    headers[NEWRELIC_TRACE_HEADER] = monisagentFormatData
...
function checkForExistingNrTraceHeaders(headers) {
  ...
  const monisagentHeader = headers[NEWRELIC_TRACE_HEADER]

  const hasExisting = traceparentHeader || monisagentHeader
  if (hasExisting) {
    logger.trace(MULTIPLE_INSERT_MESSAGE, traceparentHeader, monisagentHeader)
  }
}
...
const _payloadTest = function _payloadTest(payload) {
  if (!payload) {
...
const _configTest = function _configTest() {
  const config = this.agent.config
...
const _traceParseTest = function _traceParseTest(payload) {
...
const _traceVersionTest = function _traceVersionTest(parsed) {
...
const _requiredKeyTest = function _requiredKeyTest(data) {
  return REQUIRED_DT_KEYS.every(function checkExists(key) {
    return data[key] != null
  })
}
const _spanParentTest = function _spanParentTest(requiredKeysExist, data) {
...
const _defineAttrsFromTraceData = function _defineAttrsFromTraceData(data, transport) {
...
function _acceptDistributedTracePayload(payload, transport) {
  const payloadTest = _payloadTest.bind(this)
  ...
  const configTest = _configTest.bind(this)
  const configTestResult = configTest()
  if (!configTestResult) {
    return
  }

  const traceParseTest = _traceParseTest.bind(this)
  const parsed = traceParseTest(payload)
  if (!parsed) {
    return
  }

  const traceVersionTest = _traceVersionTest.bind(this)
  ...
  const requiredKeyTest = _requiredKeyTest.bind(this)
  const requiredKeysExist = requiredKeyTest(data)
  const spanParentTest = _spanParentTest.bind(this)
  ...
  const defineAttrsFromTraceData = _defineAttrsFromTraceData.bind(this)
