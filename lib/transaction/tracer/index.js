  ...
  function wrapped() {
    ...
    try {
      ...
      if (prev === null && process.domain != null) {
        process.domain[symbols.transactionSegment] = tracer.getSegment()
      }
...
function getOriginal(fn) {
  return fn && fn[symbols.original] ? fn[symbols.original] : fn
}

function getSegmentFromWrapped(fn) {
  return fn && fn[symbols.segment] ? fn[symbols.segment] : null
}
