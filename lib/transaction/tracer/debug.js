...
function Tracer(agent) {
  ...
  if (!dominion.available) this.context = new Context(true);
}

Tracer.prototype.getState = function () {
  if (dominion.available) {
    if (process.domain) return process.domain[PASSENGER];
  }
  else {
...
Tracer.prototype.createState = function (transaction, segment, handler) {
  ...
  if (dominion.available) dominion.add(this.agent, state);
...
Tracer.prototype.transactionProxy = function (handler) {
  ...
  var wrapped = function () {
    ...
    var value       = new NRTransaction(self.agent)
...
Tracer.prototype.segmentProxy = function (handler) {
  ...
  var wrapped = function () {
    ...
    self.enter(state, describer);
    var returned = self.monitor(handler, this, arguments, state.transaction.value);
    self.exit(state, describer);
