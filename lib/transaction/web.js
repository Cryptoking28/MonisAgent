...
var web = module.exports = {
   ...
   *
   * @param {TraceSegment} segment    The web segment.
   * @param {string}       requestURL The URL to be parsed.
   */
  setParametersFromURL : function setParametersFromURL(segment, requestURL) {
    var parsed = url.parse(requestURL, true);

    if (parsed.search !== '') {
      if (!segment.trace.parameters) segment.trace.parameters = {};
      ...
      Object.keys(parsed.query).forEach(function (key) {
        ...
        if (parsed.query[key] === '' && parsed.path.indexOf(key + '=') < 0) {
          ...
          segment.trace.parameters[key] = true;
        }
        else {
          segment.parameters[key] = parsed.query[key];
          segment.trace.parameters[key] = parsed.query[key];
        }
      });
    }
  },
  ...
  normalizeAndName : function normalizeAndName(segment, requestURL, statusCode) {
    ...
    if (statusCode === 414 || (400 <= statusCode && statusCode < 405)) {
      ...
      var name = transaction.normalizer.normalize(path);
    ...
    if (!transaction.ignore) {
      ...
      web.setParametersFromURL(segment, requestURL);
      segment.name = scope;
