...
function TraceAggregator(config) {
  ...
  /*
   * From
   * https://monisagent.atlassian.net/wiki/display/eng/Transaction+Trace+Collection+Improvements
   *
...
TraceAggregator.prototype.add = function add(transaction) {
  if (this.config.transaction_tracer && this.config.transaction_tracer.enabled &&
...
TraceAggregator.prototype.isBetter = function isBetter(name, duration, apdexT) {
  ...
  if (config &&
      ...
      config.trace_threshold !== 'apdex_f' &&
      typeof config.trace_threshold === 'number') {
    isOverThreshold = duration > config.trace_threshold;
  }
  else {
