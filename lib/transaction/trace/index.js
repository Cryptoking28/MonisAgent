...
Trace.prototype.generateSpanEvents = function generateSpanEvents() {
  ...
  const sampledOrInfinite = infiniteTracingConfigured || this.transaction.sampled
  const shouldProcess =
    sampledOrInfinite && config.span_events.enabled && config.distributed_tracing.enabled
