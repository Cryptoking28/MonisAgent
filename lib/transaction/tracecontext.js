...
class TraceContext {
  ...
  get traceparent() {
    ...
    // originating from a monisagent format DT payload being accepted.
  ...
  get tracestate() {
    ...
    if (!accountId || !appId || !trustedAccountKey) {
      ...
      this.transaction.agent.recordSupportability('TraceContext/Create/Exception')
    ...
    // this trace data is monisagent specific.
    let spanId = ''
  ...
  createTraceContextPayload() {
    ...
    if (tracestate) {
      traceContext[TRACE_CONTEXT_STATE_HEADER] = tracestate
    }
   ...
   * @property {boolean} acceptedTracestate - Whether a Monis Agent tracestate headers was
   * parsed, validated, and accepted
  ...
  acceptTraceContextPayload(traceparent, tracestate) {
    ...
    // Keep the raw, non-MonisAgent tracestate string stored so that we can propogate it
  ...
  _validateAndParseTraceStateHeader(tracestate) {
    ...
    const listMembers = tracestate.split(',').map(listMember => {
      return listMember.trim()
    }).filter((value) => {
    ...
    if (!trustedKey) {
      logger.debug(
        'Unable to accept any Monis Agent tracestate list members. ' +
      ...
      this.transaction.agent.recordSupportability('TraceContext/Accept/Exception')
    ...
    if (nrVendorIndex >= 0) {
      ...
      // Remove all matching new relic entries that we found from the vendors
      // and listMembers array
      tsd.vendors = tsd.vendors.filter(v => v !== `${trustedKey}@nr`)
    ...
    // Rebuild the new strace state string without the new relic entries
    tsd.newTraceState = listMembers.filter(v => !v.startsWith(`${trustedKey}@nr`)).join(',')
