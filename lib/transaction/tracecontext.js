...
class TraceContext {
  ...
  createTraceparent(spanContext) {
    if (spanContext) {
      return `${W3C_TRACEPARENT_VERSION}-${spanContext.traceId}-${spanContext.spanId}-0${Number(spanContext.traceFlags || 0)}`
    }
  ...
  acceptTraceContextPayload(traceparent, tracestate) {
    ...
    // Keep the raw, non-MonisAgent tracestate string stored so that we can propagate it
   ...
   * @property {string} accountId Monis Agent account ID
