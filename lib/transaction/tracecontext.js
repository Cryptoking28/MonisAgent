...
class TraceContext {
  ...
  get traceparent() {
    ...
    const traceId = this.transaction.getTraceId()
    ...
    const parentId = segment && this.transaction.agent.tracer.getSegment().id
  ...
  acceptTraceContextPayload(traceparent, tracestate, transportType) {
    ...
    if (parsedState && parsedState.entryValid) {
      ...
      this.transaction.sampled = parsedState.intrinsics.sampled
