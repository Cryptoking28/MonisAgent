...
class TraceContext {
  ...
  _validateAndParseTraceStateHeader(tracestate) {
    ...
    if (tsd.vendors.findIndex(v => v === `${trustedKey}@nr`) > -1) {
      let nrIntrinsicsValidation

      tsd.entryFound = true
      
      for (let i = 0; i < listMembers.length; i = i + 1) {
        if (listMembers[i].startsWith(`${trustedKey}@nr`)) {
          if (!tsd.entryValid) {
            const nrStateEntry = listMembers[i].split('=')[1]
            nrIntrinsicsValidation = this._validateAndParseIntrinsics(nrStateEntry)
            
            if (nrIntrinsicsValidation.entryValid) {
              tsd.entryValid = true
              tsd.intrinsics = nrIntrinsicsValidation
              delete tsd.intrinsics.entryValid
            } else {
              tsd.entryInvalidReason = nrIntrinsicsValidation.invalidReason
              tsd.entryValid = false
            }
          }
          
          // remove nr entry from listVendors
