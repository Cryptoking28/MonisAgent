...
module.exports = class TraceContext {
  ...
  get tracestate() {
    ...
    const trustedAccountKey = config.trusted_account_key || config.account_id
    const version = '0'
    const parentType = '0'  // TODO: how should we set this?
    ...
    const priority = this.transaction.priority.toFixed(6)
  ...
  addTraceConextHeaders(headers) {
    const traceContextHeaders = this.createTraceContextPayload()
    return { ...headers, traceContextHeaders }
  }
