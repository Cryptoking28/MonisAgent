...
module.exports = {
   ...
   * immediately if the error tracer is missing -- it's a Monis Agent developer
  ...
  add : function (tracer, state) {
    ...
    function errored(error) {
      ...
      // only do this in 0.9+, because o.8 domains are built on uncaughtException
      if (process._fatalException) process.emit('uncaughtException', error);
    }
