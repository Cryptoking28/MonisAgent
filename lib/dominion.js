...
module.exports = {
  ...
  add : function (agent, state) {
    ...
    // never fires in 0.10, so save the cycles -- see below for details
    if (!process._fatalException) catcher.on('error', errored);
      ...
      shimmer.wrapMethod(process, 'process', ['_fatalException'],
        ...
        return function wrapped(error) {
          ...
          if (domain.active && domain.active[MARKER]) {
            var transaction = domain.active[MARKER].getTransaction();
