...
class HarvestStep {
  ...
  /**
   * Assembles the payloads to be sent to the collector synchronously.
   *
   * @abstract
   * @protected
   */
  preparePayloadsSync(runId, datasource) { // eslint-diable-line no-unused-vars
    throw new Error(
  ...
  /**
   * Gets the harvest step ready to send synchronously.
   */
  prepareSync() {
    if (!this.datasource || this.datasource.length === 0) {
      logger.debug('No data to send to %s', this._endpoint)
    ...
    return this._payloads = this.preparePayloadsSync(
...
class Harvest {
  ...
  getPayloads() {
    ...
    return stepNames.reduce(function processStep(processedSteps, name) {
      processedSteps[METHOD_TO_ENDPOINTS[name]] = harvest._steps[name].prepareSync()
