...
class HarvestStep {
  ...
  send(callback) {
    ...
    const collector = self.harvest.agent.collector
    const self = this
    a.eachOfSeries(self._payloads, function sendEachPayload(payload, i, cb) {
      ...
      collector[self._endpoint](payload, function afterSend(err, response) {
        if (err) {
          return cb(err)
        }

        // Clean up the payload if we are not retaining it.
        if (!response.retainData) {
          self._payloads[i] = null
        }

        // TODO: Schedule retry here!
        // TODO: Signal shutdown agent here!
      })
...
class ErrorEventHarvest extends HarvestStep {
  constructor(harvest) {
    super(harvest, 'errorEvents', harvest.agent.getQueue())
