...
class HarvestStep {
  ...
  send(callback) {
    ...
    a.eachOfSeries(self._payloads, function sendEachPayload(payload, i, cb) {
      ...
      self._doSend(payload, i, function oncedCallback(err, callSite) {
        if (callbackCalled) {
          logger.info(
            'Onced callback called multiple times, this time from ' + callSite,
            err
          )
          return
        }
        callbackCalled = true
        logger.trace(
          'Onced callback called first time from ' + callSite,
          err
        )
  ...
  _doSend(payload, i, callback) {
    ...
    self._trySend(payload, function afterSend(err, response) {
      if (err) {
        return callback(err, 'trySend error')
      }
