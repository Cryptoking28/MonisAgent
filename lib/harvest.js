...
class Harvest {
  ...
  mergeUnsentData() {
    ...
    if (this._events) {
      this.agent.events.merge(this._events)
    }
  ...
  _prepareEvents() {
    ...
    const diff = this._events.overflow()
    if (diff > 0) {
      logger.warn(
  ...
  _sendMetrics(callback) {
    ...
    const payload = [this.agent.config.run_id, beginSeconds, endSeconds, metrics]
  ...
  _sendCustomEvents(callback) {
    ...
    this.agent.collector.customEvents(payload, function onCustomEvents(err) {
      if (self._shouldClearData(err, 'custom events', NAMES.CUSTOM_EVENTS)) {
  ...
  _shouldClearData(err, name, metrics) {
    if (!err) {
      return true
    } else if (err.statusCode === 413) {
      if (metrics) {
        this.agent.metrics.getOrCreateMetric(metrics.TOO_LARGE).incrementCallCount()
      }
      logger.warn('Request too large; %j data dropped.', name)
      return true
    }

    if (metrics) {
      this.agent.metrics.getOrCreateMetric(metrics.FAILED).incrementCallCount()
    }
    logger.warn('Failed to send %j; re-sampling.')
    return false
  }
}
