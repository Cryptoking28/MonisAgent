...
class HarvestStep {
  ...
  _doSend(payload, i, callback) {
    const self = this
    self.agent.collector[self._endpoint](payload, function afterSend(err, response) {
      if (err) {
        return callback(err)
      }

      // Perform the appropriate actions for the collector's response.
      if (!response.retainData) {
        self._payloads[i] = null
      } else {
        logger.info('Failed to submit data to Monis Agent, data held for redelivery.')
      }
      if (response.retryAfter && !response.shutdownAgent) {
      ...
      if (response.shutdownAgent) {
        // TODO: WHAT IS A SHUTDOWN!?
      }

      // Done!
      self.returned = response.returned
      callback()
    })
  }
...
class MetricsHarvest extends HarvestStep {
  ...
  preparePayloads(runId, metrics, callback) {
    ...
    // TODO: Move agent.mapper.load(rules) into CollectorAPI class.
  ...
  finalize(callback) {
    ...
    if (this.returned) {
      this.agent.mapper.load(this.returned)
    }
...
class TransactionTraceHarvest extends HarvestStep {
  constructor(harvest) {
    super(harvest, 'transactionSampleData', harvest.agent.traces)
    this._traces = null
  }

  preparePayloads(runId, traceAggr, callback) {
    // TODO: The harvest step shouldn't be the thing adjusting trace sampling.
    const maxTraceSegments = this.agent.config.max_trace_segments
    const traces = this._traces = [].concat(traceAggr.syntheticsTraces)
    if (traceAggr.trace) {
...
class Harvest {
  ...
  send(callback) {
    ...
    a.eachOf(this._steps, function eachHarvestStep(step, stepName, cb) {
      logger.trace('Doing harvest step %s.', stepName)
      if (!self.agent.collector.isConnected()) {
        logger.debug('Connection to Monis Agent lost during harvest.')
        return setImmediate(callback, new Error('Not connected to Monis Agent!'))
      }
      a.series([
        step.prepare.bind(step),
        step.send.bind(step),
        step.finalize.bind(step)
      ], cb)
    }, callback)
  }
}
