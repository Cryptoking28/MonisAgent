...
class Harvest {
  ...
  send(callback) {
    ...
    a.eachOf(this._steps, function eachHarvestStep(step, stepName, cb) {
    ...
    }, function afterAllHarvestSteps(err) {
      ...
      for (let stepName in self._steps) {
        if (props.hasOwn(self._steps, stepName) && self._steps[stepName].result) {
          const step = self._steps[stepName]
          if (step.result.agentRun === BEHAVIOR.SHUTDOWN) {
            agentRunAction = BEHAVIOR.SHUTDOWN
            break
          } else if (step.result.agentRun === BEHAVIOR.RESTART) {
