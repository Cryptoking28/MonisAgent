 ...
 *
 * Metrics can be "renamed", which is a process by which they are assigned a
 * short, numerical ID by Monis Agent. This can shrink the serialized JSON
 ...
 * on the fly.
 *
 * Metric names can also be normalized, which is a rule-based transform that
 * converts a metric name to another name, either to squash metric explosions,
 * or to allow developers to give more meaningful names to otherwise anonymous
 * web transactions.
 *
 * FIXME: the normalizer probably shouldn't be here -- there should only be one
 * normalizer active at any given time.
 *
 * @param {Number} apdexT The apdex-tolerating value, for use in creating apdex
 *                        statistics.
 * @param {RenameRules} renamer The mapper that turns metric names into IDs.
 * @param {MetricNormalizer} normalizer The metric name normalizer.
 */
 ...
 */
Metrics.prototype.measureMilliseconds = function (name, scope, duration, exclusive) {
 ...
 * @return {Stats} The aggregated data related to this metric.
 */
Metrics.prototype.measureBytes = function (name, size) {
 ...
 */
Metrics.prototype._getUnscopedData = function (name) {
 ...
 * @param {string} name The string to look up.
 */
Metrics.prototype._getScopedData = function (name, scope) {
 ...
 *                  for use by toJSON.
 */
Metrics.prototype._toUnscopedData = function () {
  var metricData = [];

  Object.keys(this.unscoped).forEach(function (name) {
    metricData.push(this._getUnscopedData(name));
  }, this);
