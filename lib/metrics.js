 ...
 */
function Metrics(apdexT, mapper) {
  this.started  = Date.now();
  this.apdexT   = apdexT || 0;
  this.mapper   = mapper || new MetricMapper();
...
Metrics.prototype._getUnscopedData = function (name) {
  ...
  return [this.mapper.map(name), this.unscoped[name]];
};
...
Metrics.prototype._getScopedData = function (name, scope) {
  ...
  return [this.mapper.map(name, scope), this.scoped[scope][name]];
};
