    ...
    request.on('response', function(response) {
        ...
        response.on('data', function(chunk) {
            if (response.statusCode == 200) {
            	var returnHash;
            	try {
            		returnHash = JSON.parse(chunk);
            	} catch (e) {
            		logger.debug("Error parsing chunk: " + chunk);
            		self.emit('error', method, e);
            		return;
            	}
...
function MonisAgentService(agent, fileConfig) {
    ...
    // https://github.com/wadey/node/commit/5060c83d952c818773bbf66e349604b292f73546#diff-1
