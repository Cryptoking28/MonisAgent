...
module.exports = function initialize(agent, express) {
  ...
  agent.on('transactionFinished', function transactionFinished(transaction) {
    ...
    return function cls_wrapProcessParams(layer) {
      ...
      if (layer) {
          if (layer.constructor) {
            if (!layer.constructor.prototype.handle_request[ORIGINAL]) {
              shimmer.wrapMethod(layer.constructor.prototype,
                                'express.Layer',
                                'handle_request',
                                wrapLayerHandleRequest)
            }
            if (!layer.constructor.prototype.handle_error[ORIGINAL]) {
              shimmer.wrapMethod(layer.constructor.prototype,
                                'express.Layer',
                                'handle_error',
      ...
      function wrapLayerHandleError(handleError) {
        return function wrappedLayerHandleError(error, req, res, next) {
          ...
          var transactionInfo = transactionInfoById[transaction.id]
      ...
      function wrapLayerHandleRequest(handleRequest) {
        return function wrappedLayerHandleRequest(req, res, next) {
          ...
          if (agent.config.feature_flag.express_segments) {
            ...
            if (!inRoute && !isMountedApp && routerStack.length > 0) {
              var parentRouter = routerStack[routerStack.length - 1]
              while (--parentRouter.length === 0) {
                parentRouter = routerStack.pop()
              }
