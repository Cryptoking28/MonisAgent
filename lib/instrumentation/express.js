...
function nameFromRoute(segment, route) {
  ...
  if (transaction.agent.config.capture_params && route.params) {
    var params = util._extend({}, route.params);

    // clear out ignored params
    transaction.agent.config.ignored_params.forEach(function (k) { delete params[k]; });
