...
function nameFromRoute(segment, route) {
  ...
  if (!route) return logger.error("No Express route to use for naming.");

  // put params where we can see them -- on the trace segment
  if (route.params) {
    Object.keys(route.params).forEach(function (key) {
      segment.parameters[key] = route.params[key];
    });
  }
  ...
  transaction.partialScope = NAMES.CONTROLLER + '/' + NAMES.EXPRESS.PREFIX + path;
}
  ...
  function wrapRender(version, render) {
    ...
    return function (view, options, cb, parent, sub) {
      ...
      if (cb === null || cb === undefined) {
         ...
         */
        wrapped = agent.tracer.callbackProxy(function syntheticCallback(err, rendered) {
          if (err) {
            segment.end();
            logger.trace(err,
