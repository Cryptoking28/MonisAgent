...
function nameFromRoute(segment, route, params, append) {
  if (!segment) return logger.error("No Monis Agent context to set Express route name on.")
...
module.exports = function initialize(agent, express) {
  ...
  function wrapProcessParams(version, process_params) {
    return function cls_wrapProcessParams(req) {
      if (!tracer.getTransaction()) {
        logger.trace(
      ...
      if (arguments.length) {
        if (req.route) {
          ...
          nameFromRoute(tracer.getSegment(), req.route, req.params, true)
        }
      }
      return process_params.apply(this, arguments)
    }
  ...
  function wrappedHandle(path, handle, args) {
    ...
    var original = transaction.partialName
  ...
  function addInterceptor(app) {
    ...
    if (!interceptor) {
       ...
       * we want to use the unwrapped `use` method to perserve
       * function attributes
       */
      if (app.use[ORIGINAL]) {
        app.use[ORIGINAL].call(app, '/', sentinel)
      } else {
        app.use('/', sentinel)
      }

      interceptor = app.stack.pop()
    }
    var spliced = false
  ...
  function patchEmit(parent, app) {
    ...
    function patchedEmit() {
      var result
      var router = parent._router || parent.router

      removeInterceptor(router)
      result = emit.apply(app, arguments)
      addInterceptor(router)
