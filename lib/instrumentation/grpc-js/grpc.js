...
module.exports = function instrument(shim) {
  ...
  shim.wrap(callStream.Http2CallStream.prototype, 'start', (shim, original) => {
    return function wrappedStart() {
      ...
      const authorityName = channel.defaultAuthority

      const segment = shim.createSegment({
        name: `External/${authorityName}${this.methodName}`,
        opaque: true,
        recorder: recordExternal(authorityName, this.methodName)
      })
      ...
      function callStart() {
        ...
        const nrListener = {
          onReceiveMetadata: (metadata) => {
            originalListener && originalListener.onReceiveMetadata(metadata)
          },
          onReceiveMessage: (message) => {
            originalListener && originalListener.onReceiveMessage(message)
          },
...
function recordExternal(host, method) {
  return function externalRecorder(segment, scope) {
    const duration = segment.getDurationInMillis()
    const exclusive = segment.getExclusiveDurationInMillis()
    const transaction = segment.transaction
    const metricName = `${EXTERNAL.PREFIX}${host}${method}`
