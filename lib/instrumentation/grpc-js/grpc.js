...
function wrapRegister(shim, original) {
  ...
  return function wrappedRegister() {
    ...
    // We need the server for the host and port, but depending on how
    // the user calls server functions, the connection might not be
    // ready yet. It will be ready below by the time the registered
    // handler is invoked.
    const server = this

    if (this.handlers.has(name)) {
      shim.logger.debug(
        `not instrumenting again gRPC method handler for ${name}: it is already registered in the server`
      )
      return original.apply(this, arguments)
    }

    args[1] = shim.bindCreateTransaction(createTransaction, { type: shim.WEB })

    return original.apply(this, args)

    function createTransaction(stream) {
      const parent = shim.getActiveSegment()
      const transaction = parent.transaction
      const address = server.http2ServerList[0].channelzRef.name
      ...
      headers.monisagent = metadata.get('monisagent').join(',')
      ...
      // client is streaming. https://issues.monisagent.com/browse/NEWRELIC-1460
