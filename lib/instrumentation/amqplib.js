...
// monisagent.instrumentMessages('amqplib', instrumentChannelAPI)
// monisagent.instrumentMessages('amqplib/channel_api', instrumentChannelAPI)
// monisagent.instrumentMessages('amqplib/channel_api.js', instrumentChannelAPI)
// monisagent.instrumentMessages('amqplib/callback_api', instrumentCallbackAPI)
// monisagent.instrumentMessages('amqplib/callback_api.js', instrumentCallbackAPI)
...
function wrapChannel(shim) {
  ...
  function recordSendMessage(shim, fn, n, args) {
    ...
    var exchange = isDefault ? 'Default'
      : TEMP_RE.test(fields.exchange) ? null
      : fields.exchange
    ...
    return {
      ...
      parameters: getParameters({}, fields)
    }
  }
}
...
function wrapPromiseChannel(shim) {
  ...
  shim.recordConsume(proto, 'get', {
    ...
    promise: true,
    resultHandler: function handleConsumedMessage(shim, fn, name, message) {
      if (!message) {
      ...
      return {parameters: parameters, messageProperties: headers}
    }
  })
...
function wrapCallbackChannel(shim) {
  ...
  shim.recordConsume(proto, 'get', {
    destinationName: shim.FIRST,
    callback: shim.LAST,
    resultHandler: function handleConsumedMessage(shim, fn, name, args) {
      var message = args[1]
      if (!message) {
        shim.logger.trace('No results from consume.')
        return null
...
function makeRecordConsume(queue) {
  return function recordConsume(shim, fn, name, args) {
    var message = args[0]
    ...
    var parameters = getParameters({queue_name: queue}, message.fields)
    getParameters(parameters, message.properties)
