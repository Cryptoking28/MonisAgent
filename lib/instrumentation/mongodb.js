 ...
 * operation, a cursor operation, or a collection operation.
 */
 ...
 *
 * The following functions enable registering, de-registering, and checking if we
  ...
  return function cls_wrapCursorOperation(operation) {
    return tracer.segmentProxy(function mongoCursorOperationProxy() {
      ...
      if (!tracer.getTransaction()) {
        logger.trace('Not tracing MongoDB %s.%s(%s); no Monis Agent transaction.',
      ...
      if (typeof callback !== 'function') {
        logger.trace('Not tracing MongoDB %s.%s(%s); last argument was not a callback.',
                     collection, operationName, terms)
        return operation.apply(this, arguments)
      }
      ...
      if (currentIsntMongo ||
        ...
        logger.trace('Tracing MongoDB %s.%s(%s).', collection, operation, terms)
        ...
        logger.trace('Not tracing MongoDB %s.%s(%s); MongoDB segment already in progress.',
                     collection, operationName, terms)
      }
      ...
      args[last] = tracer.callbackProxy(function cursorOperationCbProxy() {
        ...
        if (segment && cursor.state === cursor.constructor.CLOSED) {
          logger.trace('Tracing MongoDB %s.%s(%s) ended.',
    ...
    shimmer.wrapMethod(mongodb.Collection.prototype,
      ...
      return tracer.segmentProxy(function cb_segmentProxy() {
        ...
        if (args.length < 1) {
          ...
          logger.trace('Not tracing MongoDB %s.%s(); no Monis Agent transaction.',
                       collection, operation)
        ...
        if (!isCurrentSegmentCollectionOp(tracer) ||
            isCurrentSegmentInCallbackState(tracer)) {
          logger.trace('Tracing MongoDB %s.%s(%s).', collection, operation,
                       terms)
