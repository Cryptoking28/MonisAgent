...
const createFormatter = require('@monisagent/winston-enricher/lib/createFormatter')
const API = require('../../api')

module.exports = function instrument(agent, winston, _, shim) {
  const config = agent.config
  if (!config.application_logging || !config.application_logging.enabled) {
    shim.logger.debug('Application logging not enabled. Not instrumenting winston...')
    return // do not instrument logs if logging is not enabled
  }
