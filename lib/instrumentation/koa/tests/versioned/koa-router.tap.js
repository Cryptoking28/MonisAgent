...
tap.test('koa-router instrumentation', function(t) {
  ...
  t.test(
    ...
    function(t) {
      ...
      router.get('/:third', function secondMiddleware(ctx, next) {
      })
      app.use(router.routes())
      helper.agent.on('transactionFinished', function(tx) {
        t.exactSegments(
  ...
  t.test('should work with multiple routers', function(t) {
    ...
    router2.get('/:second', function secondMiddleware(ctx, next) {
      ctx.body += ' second'
    })
    app.use(router.routes())
  ...
  t.test('should work with an array of paths', function(t) {
    ...
    router.get('/:second', function terminalMiddleware(ctx, next) {
      ctx.body = ' second'
    })
    app.use(router.routes())
    helper.agent.on('transactionFinished', function(tx) {
