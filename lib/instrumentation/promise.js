 ...
 *
 * @param {Agent}         agent   - The Monis Agent APM agent.
...
module.exports = function initialize(agent, library, spec) {
  ...
  function wrapStaticMethods(lib, name, staticSpec) {
    ...
    _safeWrap(lib, name, staticSpec.resolve, wrapResolve)
    _safeWrap(lib, name, staticSpec.reject, wrapReject)
  }
  ...
  function wrapExecutorCaller(caller) {
    return function wrappedExecutorCaller(executor) {
      ...
      try {
        return executor.apply(context.self, context.args)
      } catch (err) {
        context.args[1](err)
      }
    }
   ...
   * @param {Promise}   next  Promise returned from calling the *then*
   ...
   */
  function wrapThen(then) {
    return function wrappedThen() {
      var thenSegment = agent.tracer.getSegment()
      var promise = this

      // wrap arguments that are functions
      ...
      function wrapHandler(fn, i) {
        ...
        return function wrappedHandler() {
          ...
          return promise.__NR_wrapper(this, fn, next, arguments, promise)
        }
      }
    }
  }
  ...
  /**
   * Creates a wrapper around the static resolve factory method.
   */
  function wrapResolve(resolve, name) {
    return function wrappedResolve(val) {
      var segment = _createSegment('Promise.' + name)
      var prom = resolve.call(this, val)
  ...
  /**
   * Creates a wrapper around the static reject factory method.
   */
  function wrapReject(reject, name) {
    return function wrappedReject(reason) {
      var segment = _createSegment('Promise.' + name)
      var prom = reject.call(this, reason)
      bindLink(prom, segment)
      return prom
    }
  }
