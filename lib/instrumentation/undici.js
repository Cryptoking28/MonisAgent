...
module.exports = function addUndiciChannels(agent, undici, modName, shim) {
  ...
  diagnosticsChannel.channel('undici:request:headers').subscribe(({ request, response }) => {
    ...
    if (
      ...
      cat.pullCatHeaders(agent.config, activeSegment, appData)
      try {
        const attrs = activeSegment.getAttributes()
        const url = new URL(attrs.url)
        activeSegment.name = `${NAMES.EXTERNAL.TRANSACTION}${url.host}/${activeSegment.catId}/${activeSegment.catTransaction}`
      } catch(err) {
        logger.warn(err, 'Cannot rename segment for CAT')
      }
    }
  })
  ...
  function endAndRestoreSegment(ctx, error) {
    const activeSegment = ctx[SEGMENT]
    const parentSegment = ctx[PARENT_SEGMENT]
    if (activeSegment) {
      activeSegment.end()
