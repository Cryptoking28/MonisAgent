...
function nameFromRequest(segment, request) {
  if (!segment) return logger.error("No Monis Agent context to set Hapi route name on.");
  ...
  if (transaction.agent.config.capture_params && request.params) {
    var params = util._extend({}, request.params);
...
module.exports = function initialize(agent, hapi) {
  ...
  shimmer.wrapMethod(
    hapi && hapi.Server && hapi.Server.prototype,
