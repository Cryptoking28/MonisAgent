...
module.exports = function initialize(agent, restify, moduleName, shim) {
  ...
  function wrapCreateServer(s, fn, fnName, server) {
  ...
  function wrapServer(serverProto) {
    ...
    shim.recordArgsAsMiddleware(serverProto, 'use')
  }
}


function nameFromRoute(segment, route, context) {
  if (!segment) return logger.error("No Monis Agent context to set Restify route name on.")
