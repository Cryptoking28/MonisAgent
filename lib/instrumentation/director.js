...
function nameTransaction(segment, partialName, res) {
  if (!segment) return logger.error("No Monis Agent context to set Director route name on.")
  ...
  if (res.__NR_directored) { // not first route
    nameState.popPath() // replace latest path name, preserving the last path name
  }
