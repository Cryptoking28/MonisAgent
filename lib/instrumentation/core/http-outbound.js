...
module.exports = function instrumentOutbound(agent, opts, makeRequest) {
  ...
  function instrumentRequest(segment) {
    ...
    const dtDisabled = opts.headers && opts.headers[SHIM_SYMBOLS.DISABLE_DT]
    if (agent.config.cross_application_tracer.enabled && !dtDisabled) {
      ...
      logger.trace({
        config: agent.config.cross_application_tracer.enabled,
        symbol: !!dtDisabled
      }, 'CAT disabled, not adding headers!')
    }
