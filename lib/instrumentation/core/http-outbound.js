...
const NEWRELIC_ID_HEADER = 'x-monisagent-id'
const NEWRELIC_TRACE_HEADER = 'monisagent'
const NEWRELIC_TRANSACTION_HEADER = 'x-monisagent-transaction'
  ...
  function instrumentRequest(segment) {
    ...
    if (agent.config.distributed_tracing.enabled) {
      if (opts.headers && opts.headers[SHIM_SYMBOLS.DISABLE_DT]) {
        ...
        if (agent.config.feature_flag.dt_format_w3c) {
          _addTraceContextHeaders(transaction, outboundHeaders)
        }
        // TODO: may need new config item: distributed_tracing.include_new_relic_payload
        // to gate the DT headers
 ...
 * @param {Error} error
 *
 * @return {bool} True if the error will be collected by Monis Agent.
 */
...
function handleResponse(segment, hostname, req, res) {
  ...
  if (
    ...
    pullCatHeaders(agent.config, segment, hostname, res.headers['x-monisagent-app-data'])
  }
...
function pullCatHeaders(config, segment, host, obfAppData) {
  ...
  if (!obfAppData) {
    logger.trace('Got no CAT app data in response header x-monisagent-app-data')
  } else {
    var appData = null
    try {
      appData =
      ...
      logger.warn('Got an unparsable CAT header x-monisagent-app-data: %s', obfAppData)
      return
    }
...
function _addTraceContextHeaders(transaction, outboundHeaders) {
  if (!outboundHeaders[TRACE_CONTEXT_PARENT_HEADER]) {
    const traceContextHeaders = transaction.createTraceContextHeaders()
    outboundHeaders[TRACE_CONTEXT_PARENT_HEADER] = traceContextHeaders.parent
