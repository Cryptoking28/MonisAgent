...
function maybeAddDtCatHeaders(agent, transaction, outboundHeaders, headers = {}) {
  if (agent.config.distributed_tracing.enabled) {
    if (!!(headers[symbols.disableDT] || headers['x-monis-agent-disable-dt'])) {
      ...
      // see: https://github.com/Cryptoking28/monisagent-aws-sdk/issues/168
      delete headers['x-monis-agent-disable-dt']
    } else {
 ...
 * @returns {boolean} True if the error will be collected by Monis Agent.
 */
