 ...
 *
 */
var SHOULD_WRAP_HTTPS = psemver.satisfies('<0.11 || >=9.0.0 || 8.9.0')
...
function wrapEmitWithTransaction(agent, emit) {
  ...
  return tracer.transactionProxy(function wrappedHandler(evnt, request, response) {
    ...
    transaction.addAgentAttribute('request.uri', transaction.url)
      ...
      if (response) {
        if (response.statusCode != null) {
          ...
          if (/^\d+$/.test(responseCode)) {
            transaction.addAgentAttribute('response.status', responseCode)
          }
        }
...
function _collectHeaders(headers, nameMap, prefix, tx) {
  ...
  for (var i = 0; i < headerKeys.length; i++) {
    ...
    if (header !== undefined) {
      ...
      tx.addAgentAttribute(attributeName, header)
    }
  }
}
