...
module.exports = function initialize(agent, http, moduleName) {
  ...
  // requests. From 0.11 until 8.9, the `https` module simply called through to
  // the `http` methods, so to prevent double-instrumenting we need to check
  // what module we're instrumenting and what version of Node we're on.
  if (moduleName !== 'https' || psemver.satisfies('<0.11 || >=8.9.0')) {
    shimmer.wrapMethod(
      http,
      'http',
      'request',
