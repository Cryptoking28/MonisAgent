...
const NEWRELIC_APP_DATA_HEADER = 'x-monisagent-app-data'
...
const TRACE_CONTEXT_PARENT_HEADER = 'parent'
...
function wrapEmitWithTransaction(agent, emit, isHTTPS) {
  ...
  return tracer.transactionProxy(function wrappedHandler(evnt, request, response) {
    ...
    if (agent.config.distributed_tracing.enabled) {
      // TODO: W3C fork based on format
      // inbound request -- propagate headers here
      // accept both headers here.. only propagate w3c if present
      // else propagate monisagent headers
      // do we validate w3c header here?
      if (agent.config.feature_flag.dt_format_w3c) {
        const parent = request.headers[TRACE_CONTEXT_PARENT_HEADER]
        if (parent) {
          logger.trace(
            'Accepting trace context payload for transaction %s',
            transaction.id
          )
          transaction.acceptTraceContextPayload(parent, '', transport)
        }
      }
