...
function wrapListener(agent, listener) {
  ...
  return tracer.transactionProxy(function wrappedHandler(request, response) {
      ...
      if (encKey) {
        ...
        if (obfTransaction) {
          ...
          try {
            ...
            logger.trace(
              'Got an unparsable CAT header x-monisagent-transaction: %s',
              obfTransaction
            )
          }
          if (externalTrans) {
    ...
    function instrumentedFinish() {
       ...
       */
      transaction.setName(parsedUrl, response.statusCode)

      // This should be the last thing called before the web segment finishes.
      segment.markAsWeb(parsedUrl)
  ...
  return function wrappedRequest(options, callback) {
    ...
    if (transaction && !internalOnly) {
      if (util.isArray(options.headers)) {
        ...
        requested = request.apply(this, args)
        setHeaders()
      }

      requested = requested || request.apply(this, args)
      ...
      segment = instrumentOutbound(agent, requested, hostname, port)
    }

    return requested || request.apply(this, args)

    function wrappedCallback() {
      if (!segment) return callback.apply(this, arguments)
      return tracer.bindFunction(callback, segment).apply(this, arguments)
    }
