 ...
 *
 */
var SHOULD_WRAP_HTTPS = psemver.satisfies('>=9.0.0 || 8.9.0')
var NR_CONNECTION_PROP = '__NR__connection'
var DEFAULT_HOST = 'localhost'
var DEFAULT_PORT = 80
var REQUEST_HEADER = 'x-request-start'
var QUEUE_HEADER = 'x-queue-start'
var NEWRELIC_ID_HEADER = 'x-monisagent-id'
var NEWRELIC_APP_DATA_HEADER = 'x-monisagent-app-data'
const NEWRELIC_TRACE_HEADER = 'x-monisagent-trace'
var NEWRELIC_TRANSACTION_HEADER = 'x-monisagent-transaction'
var NEWRELIC_SYNTHETICS_HEADER = 'x-monisagent-synthetics'
...
var HEADER_ATTR_NAMES = {
  ...
  'x-monisagent-app-data':          'xNewrelicAppData',
  'x-monisagent-id':                'xNewrelicId',
  'x-monisagent-synthetics':        'xNewrelicSynthetics',
  'x-monisagent-trace':             'xNewrelicTrace',
  'x-monisagent-transaction':       'xNewrelicTransaction',
...
var REQUEST_HEADER_PREFIX = 'request.headers.'
var RESPONSE_HEADER_PREFIX = 'response.headers.'
var REQUEST_HEADER_NAMES = Object.create(null)
var RESPONSE_HEADER_NAMES = Object.create(null)
...
function wrapRequest(agent, request) {
  ...
  return function wrappedRequest(options) {
    var tracer = agent.tracer
    var transaction = tracer.getTransaction()
    var outboundHeaders = Object.create(null)
    ...
    if (transaction && !internalOnly) {
      ...
      if (agent.config.cross_application_tracer.enabled) {
        ...
        if (agent.config.feature_flag.distributed_tracing) {
          txHeader = NEWRELIC_TRACE_HEADER

          try {
            txData = transaction.createDistributedTracePayload().httpSafe()
          ...
          txHeader = NEWRELIC_TRANSACTION_HEADER
        ...
        if (txData) {
          outboundHeaders[txHeader] = txData
        }
      }
    }
    ...
    } else if (logger.traceEnabled() && !transaction && !internalOnly) {
      var logOpts = typeof options === 'string' ? url.parse(options) : options
      logger.trace(
...
/**
 * Take the X-MonisAgent-Synthetics header and apply any appropriate data to the
 * transaction for later use. This is the gate keeper for attributes being
 * added onto the transaction object for synthetics.
 *
 * @param {string} header - The raw X-MonisAgent-Synthetics header
...
/**
 * Parse out and verify the the pieces of the X-MonisAgent-Synthetics header.
 *
 * @param {string} header - The raw X-MonisAgent-Synthetics header
 * @param {string} encKey - Encoding key handed down from the server
 * @param {̄Array} trustedIds - Array of accounts to trust the header from.
