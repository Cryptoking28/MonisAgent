...
var shimmer            = require('../../shimmer.js')
  , logger             = require('../../logger').child({component : 'http'})
  , recordWeb          = require('../../metrics/recorders/http.js')
  , hashes             = require('../../util/hashes.js')
      ...
      if (encKey) {
        ...
        if (obfTransaction) {
          ...
          try {
            ...
            logger.warn('Got an unparsable CAT header x-monisagent-transaction: %s',
                        obfTransaction);
          }
          if (externalTrans) {
...
function wrapWriteHead(agent, writeHead) {
  return function wrappedWriteHead() {
    ...
    // Stored on the tx so we can push a metic with this time instead of
    // actual duration.
...
function wrapRequest(agent, request) {
  return agent.tracer.segmentProxy(function wrappedRequest(options, callback) {
    ...
    if (internalOnly) options[NR_CONNECTION_PROP] = undefined;
      ...
      // TODO: cat-test On an outbound request in the middle of a transaction,
      // we just need to make sure we have these headers and they are the
      // expected values.
    ...
    if (transaction && !internalOnly) {
      // This is only hit on a cat request and should append a couple headers.
