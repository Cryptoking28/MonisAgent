...
function _getErrorEventIntrinsicAttrs(transaction, errorClass, message, expected, timestamp, conf) {
  ...
  if (transaction) {
    ...
    AttributeHelper.maybeAddQueueAttributes(transaction, attributes)
    AttributeHelper.maybeAddExternalAttributes(transaction, attributes)
    AttributeHelper.maybeAddDatabaseAttributes(transaction, attributes)
    AttributeHelper.maybeAddSyntheticAttributes(transaction, attributes)
