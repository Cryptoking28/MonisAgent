...
const { array, int, float, boolean, object, objectList } = require('./formatters')
...
defaultConfig.definition = () => ({
   ...
   * token from the Monis Agent Admin below.
   */
   ...
   *
   * @see https://docs.monisagent.com/docs/apm/monis-agent-apm/apdex/changing-your-apdex-settings
   */
  apdex_t: {
    formatter: float,
    default: 0.1
  ...
  logging: {
     ...
     */
    filepath: {
      default: require('path').join(process.cwd(), 'monisagent_agent.log'),
      env: 'NEW_RELIC_LOG'
    },
    /**
  ...
  error_collector: {
    ...
    expected_messages: {
      // TODO: how to make this env var compliant?
      formatter: object,
      default: {}
    },
  ...
  transaction_tracer: {
    ...
    record_sql: {
      formatter(val) {
        return ['off', 'obfuscated', 'raw'].includes(val) ? val : 'off'
      },
      default: 'off',
  ...
  distributed_tracing: {
    ...
    /**
     * Excludes Monis Agent format distributed tracing header (`monisagent`) on
     * outbound requests when set to `true`. By default (when false)
     * both W3C TraceContext (`traceparent`, `tracecontext`) and
     * Monis Agent formats will be sent.
     */
    exclude_monisagent_header: {
      formatter: boolean,
      default: false
    }
  },
  ...
  application_logging: {
    ...
    forwarding: {
      ...
      /**
       * Number of log records to send per minute to Monis Agent.
       */
      max_samples_stored: {
        formatter: int,
        default: 10000
   ...
   * disallow the use of Monis Agent's server-side configuration for agents.
...
function buildConfig(definition, config, nestedKeys = [], len = 1) {
  let actualLen = 0
  Object.entries(definition).reduce((conf, [key, value]) => {
    actualLen++
    if (typeof value === 'string') {
      if (nestedKeys.length) {
        setNestedKey(conf, [...nestedKeys, key], value)
      } else {
        conf[key] = value
      }
    } else if (value.hasOwnProperty('default')) {
      if (nestedKeys.length) {
        setNestedKey(conf, [...nestedKeys, key], value.default)
      } else {
      ...
      nestedKeys.push(key)
      buildConfig(definition[key], conf, nestedKeys, length)
    }
