...
exports.config = {
  ...
  port: 443,
  /**
   * Whether or not to use SSL to connect to Monis Agent's servers.
   *
   ...
   * disallow the use of Monis Agent's server-side configuration. To do so, set
   * this parameter to true. Some configuration information is required to make
   * the module work properly with the rest of Monis Agent, but settings such as
   * apdex_t and capture_params will not be override-able by Monis Agent with this
   * setting in effect.
   *
   ...
   * lower than Monis Agent standard, but Node.js applications tend to be more
   ...
   */
  // TODO: logic around default attributes.exclude headers
  ...
  attributes: {
    /**
     * Whether or not attributes should be collected at all.
     *
     * Equivalent to `config.attributes.enabled`.
     */
    enabled: true,
    /**
     ...
     *
     * @env NEW_RELIC_IGNORED_ATTRIBUTES
     */
    exclude: [
      // TODO
    ...
    /**
     * Array of parameters you want captured off request URLs in
     * transaction and error traces.
     *
     * @env NEW_RELIC_INCLUDED_ATTRIBUTES
     */
    include: []
  },
  /**
   ...
   * transaction traces and error traces.
   *
   * Equivalent to `config.attributes.exclude`.
   *
   * @env NEW_RELIC_IGNORED_PARAMS
   */
  // TODO: make equivalent to attributes.exclude
  ignored_params: [],
  logging: {
    /**
     ...
     * @env NEW_RELIC_LOG
     */
    filepath: require('path').join(process.cwd(), 'monisagent_agent.log'),
    /**
  ...
  error_collector: {
    ...
    attributes: {
      /**
       * @env NEW_RELIC_ERROR_COLLECTOR_ATTRIBUTES
       */
      enabled: true,
      /**
       *
     ...
     * and sent to Monis Agent -- it DOES NOT remove any instrumentation.
     */
    enabled: true,
    /**
     ...
     * grows too large the Monis Agent servers may reject it.
     */
  ...
  transaction_tracer: {
    ...
    attributes: {
       ...
       *
       *
       * @env NEW_RELIC_TRACER_ATTRIBUTES
       */
      enabled: true,
      /**
       *
    ...
    /**
     * Whether to collect & submit slow transaction traces to Monis Agent. The
  ...
  debug: {
     ...
     * small number of people inside Monis Agent, so you should probably only
     * enable tracer tracing if asked to by Monis Agent, because it will affect
  ...
  rules: {
     ...
     * matching incoming request URLs and naming the associated Monis Agent
   ...
   * this setting unless you understand the implications of Monis Agent's
  ...
  browser_monitoring: {
     ...
     * The monisagent module can generate the appropriate <script> header, but you must
     ...
     *       var header = monisagent.getBrowserTimingHeader();
     *       res.write(header)
   ...
   * Transaction Events
   *
   * Transaction events are sent to Monis Agent Insights. This event data
   * includes transaction timing, transaction name, and any custom parameters.
   *
   * Read more here: http://monisagent.com/insights
   */
  transaction_events: {
    /**
     *
   ...
   *
   * Custom insights events are JSON object that are sent to Monis Agent
   * Insights. You can tell the agent to send your custom events via the
   * `monisagent.recordCustomEvent()` API. These events are sampled once the max
   ...
   * being sent to Monis Agent. The local setting must match the server setting.
   ...
   *
   * To read more see: https://docs.monisagent.com/docs/subscriptions/high-security
   */
  high_security: false,
