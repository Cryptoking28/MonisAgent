...
Config.prototype._featureFlagFromEnv = function _featureFlagFromEnv() {
  Object.keys(feature_flag.prerelease).concat(feature_flag.released).forEach(key => {
    const envVal = process.env['NEW_RELIC_FEATURE_FLAG_' + key.toUpperCase()]
    if (envVal) {
      this.feature_flag[key] = isTruthular(envVal)
    }
  })
}
