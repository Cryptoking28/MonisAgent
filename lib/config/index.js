...
Config.prototype._DTManuallySet = function _DTManuallySet(inputConfig) {
  ...
  return (inputEnabled || process.env.NEW_RELIC_DISTRIBUTED_TRACING_ENABLED) !== undefined
}
...
Config.prototype._enforceServerless = function _enforceServerless(inputConfig) {
  if (this.serverless_mode.enabled) {
    ...
    if (!this._DTManuallySet(inputConfig) && this.account_id) {
      this.distributed_tracing.enabled = true
    } else if (!this._DTManuallySet(inputConfig) && !this.account_id) {
      logger.info(
        'To use distributed tracing you will need to add an account_id ' +
        'either in your monisagent.js file or via environment variables.'
      )
    }
