...
function Config(config) {
  ...
  // 8. If DT and lambda_mode are set, enforce that DT config values are set.
  if (this.distributed_tracing.enabled && this.lambda_mode) {
    this._enforceLambdaDT()
  }
...
/**
 * Verifies that config values required for using distributed tracing
 * with AWS Lambda have been defined.
 */
Config.prototype._enforceLambdaDT = function _enforceLambdaDT() {
  if (!(this.trusted_account_key && this.application_id && this.account_id)) {
    const REQUIRED_KEYS = ['trusted_account_key', 'application_id', 'account_id']

    throw new Error(
      'Using distributed tracing in AWS Lambda requires the following ' +
      'config values be defined, either in your monisagent.js file ' +
      'or via environment variables: ' + REQUIRED_KEYS.join(', ') + '.'
    )
  }
}
...
Config.prototype._canonicalize = function _canonicalize() {
  ...
  // only try detecting lambda_mode if user hasn't explicitly set it
  if (_autoDetectLambda) {
    this.lambda_mode = process.env.AWS_LAMBDA_FUNCTION_NAME != null
  }
