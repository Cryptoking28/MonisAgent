...
function Config(config) {
  ...
  // 8. lambda_mode specific config setting
...
/**
 * Verifies/cleans data specific to running DT in lambda_mode.
 */
Config.prototype._enforceLambdaDT = function _enforceLambdaDT() {
  const DT_KEYS = ['trusted_account_key', 'application_id', 'account_id']
  if (this.lambda_mode) {
    // If DT and lambda_mode are set, enforce that DT config values are set.
    const allSet = DT_KEYS.reduce((acc, key) => {
      return acc && this[key]
    }, true)
    ...
    // Don't allow DT config settings to be set if lambda_mode is disabled
    DT_KEYS.forEach((key) => {
      if (this[key] != null) {
        logger.warn(
          key + ' was configured locally without enabling lambda_mode. ' + 
          'This value will be cleared and set by the Monis Agent servers.'
        )
        this[key] = null
      }
    })
