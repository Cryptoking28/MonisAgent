...
const getConfigFileNames = () =>
  [process.env.NEW_RELIC_CONFIG_FILENAME, 'monisagent.js', 'monisagent.cjs', 'monisagent.mjs'].filter(
    Boolean
  )
 ...
 * e.g. the configuration from the Monis Agent server has overridden some
 * local setting.
 *
 * @event Config#change
 ...
 *
 * @param {object} config Configuration object as read from a `monisagent.js`
 ...
 * setting of high security mode and what the Monis Agent server has returned.
 * @fires Config#change
 */
...
Config.prototype.logUnknown = function logUnknown(json, key) {
  const value = json[key]
  logger.debug('Monis Agent sent unknown configuration parameter %s with value %s.', key, value)
}
...
function initialize(config) {
  ...
  try {
    ...
    if (error.code === 'ERR_REQUIRE_ESM') {
      // Attempted to import monisagent.js or similar from an ESM module, error out early.
      throw error
    }
    logger.error(error)
...
function createNewConfigObject(config) {
  ...
  if (config.monisagent_home) {
    delete config.monisagent_home
  }
  return config
}
