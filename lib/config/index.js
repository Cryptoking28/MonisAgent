 ...
 */
var DEFAULT_CONFIG_PATH = require.resolve('./default')
var DEFAULT_CONFIG = require('./default').config
var DEFAULT_FILENAME = 'monisagent.js'
 ...
 */
var ENV_MAPPING = {
  monisagent_home: "NEW_RELIC_HOME",
  ...
  capture_params: "NEW_RELIC_CAPTURE_PARAMS", // TODO
  ignored_params: "NEW_RELIC_IGNORED_PARAMS", // TODO
  attributes: {
    enabled: 'NEW_RELIC_CAPTURE_ATTRIBUTES',
    exclude: "NEW_RELIC_IGNORED_ATTRIBUTES",
    include: "NEW_RELIC_INCLUDED_ATTRIBUTES"
  },
  logging: {
  ...
  error_collector: {
    attributes: {
      enabled: "NEW_RELIC_ERROR_COLLECTOR_ATTRIBUTES"
    },
  ...
  transaction_tracer: {
    attributes: {
      enabled: "NEW_RELIC_TRACER_ATTRIBUTES"
    },
  ...
  browser_monitoring: {
    attributes: {
      enabled: "NEW_RELIC_BROWSER_MONITOR_ATTRIBUTES"
    },
...
var LIST_VARS = [
  'NEW_RELIC_APP_NAME',
  'NEW_RELIC_IGNORED_PARAMS', // TODO
  'NEW_RELIC_IGNORED_ATTRIBUTES',
  'NEW_RELIC_INCLUDED_ATTRIBUTES',
 ...
 * don't get fancy--just use 'true' and 'false', everybody.
 */
var BOOLEAN_VARS = [
  "NEW_RELIC_IGNORE_SERVER_CONFIGURATION",
  "NEW_RELIC_ENABLED",
  "NEW_RELIC_CAPTURE_PARAMS", // TODO
  'NEW_RELIC_CAPTURE_ATTRIBUTES',
  "NEW_RELIC_ERROR_COLLECTOR_ENABLED", // TODO
  'NEW_RELIC_ERROR_COLLECTOR_ATTRIBUTES',
  "NEW_RELIC_TRACER_ENABLED", // TODO
  'NEW_RELIC_TRACER_ATTRIBUTES',
  "NEW_RELIC_DEBUG_METRICS",
  "NEW_RELIC_DEBUG_TRACER",
  "NEW_RELIC_ENFORCE_BACKSTOP",
  "NEW_RELIC_USE_SSL",
  "NEW_RELIC_BROWSER_MONITOR_ENABLE", // TODO
  'NEW_RELIC_BROWSER_MONITOR_ATTRIBUTES',
...
function fromObjectList(setting) {
  try {
    ...
    logger.error("Monis Agent configurator could not deserialize object list:")
...
function Config(config) {
  ...
  // TODO stil not sure where exactly this comes in
 ...
 *
 * @param {object} json The config blob sent by Monis Agent.
 */
...
Config.prototype._fromServer = function _fromServer(params, key) {
  switch (key) {
    ...
    case 'data_report_period':
      ...
      break
    case 'ignored_params': // TODO: port to attributes.exclude
      this._updateIfChanged(params, key)
      break
    case 'attributes.exclude':
      ...
      break
    case 'transaction_tracer.enabled': // TODO: port to transaction_tracer.attributes.enabled
      this._updateNestedIfChanged(
        params,
    ...
    case 'transaction_tracer.attributes.enabled':
      this._updateNestedIfChanged(
        params,
        this.transaction_tracer.attributes,
        key,
        'enabled'
      )
      break
    case 'transaction_tracer.transaction_threshold':
      ...
      break
    case 'error_collector.enabled': // TODO: port to error_collector.attributes.enabled
      this._updateNestedIfChanged(
        params,
        this.error_collector,
    ...
    case 'collect_analytics_events':
      ...
      if (params.collect_analytics_events === false)
        this.transaction_events.enabled = false // TODO: port to transaction_events.attributes.enable
        this.transaction_events.attributes.enabled = false
      break
    ...
    case 'transaction_events.attributes.enabled':
      this._updateNestedIfChanged(
        params,
        this.transaction_events.attributes,
        key,
        'enabled'
      )
      break
    case 'transaction_events.enabled': // TODO: port to transaction_events.attributes.enabled
      this._updateNestedIfChanged(
        params,
        this.transaction_events,
        key,
...
Config.prototype._alwaysUpdateIfChanged = function _alwaysUpdateIfChanged(json, key) {
  ...
  if (value != null && this[key] !== value) {
    ...
    this.emit(key, value)
    logger.debug('Configuration of %s was changed to %s by Monis Agent.', key, value)
  }
}
 ...
 *
 * @param {object} remote    JSON sent from Monis Agent.
 * @param {object} local     A portion of this configuration object.
 * @param {string} remoteKey The name sent by Monis Agent.
...
function _updateNestedIfChangedRaw(remote, local, remoteKey, localKey) {
  ...
  if (value != null && local[localKey] !== value) {
    ...
    logger.debug('Configuration of %s was changed to %s by Monis Agent.', remoteKey, value)
  }
}
...
Config.prototype.logUnknown = function logUnknown(json, key) {
  var value = json[key]
  logger.debug(
    "Monis Agent sent unknown configuration parameter %s with value %s.",
    key,
    value
  )
}
 ...
 * it sends the extra variable to Monis Agent, which is not desired.
 *
...
Config.prototype._canonicalize = function _canonicalize() {
  ...
  this.attributes.exclude = this.attributes.exclude.length
                              ...
                              : this.ignored_params
  this.attributes.enabled = _chooseBool(this.attributes.enabled, this.capture_params)
  this.transaction_events.attributes.enabled = _chooseBool(
    this.transaction_events.attributes.enabled,
    this.transaction_events.enabled
  )
  this.transaction_tracer.attributes.enabled = _chooseBool(
    this.transaction_tracer.attributes.enabled,
    this.transaction_tracer.enabled
  )
  this.error_collector.attributes.enabled = _chooseBool(
    this.error_collector.attributes.enabled,
    this.error_collector.enabled
  )
  this.browser_monitoring.attributes.enabled = _chooseBool(
    this.browser_monitoring.attributes.enabled,
    this.browser_monitoring.enable
  )
}

// TODO: not accurate if browser_monitoring.attributes.enabled defaults to false
function _chooseBool(cur, old) {
  // If both vals are equal, return the new one.
  // If not, return the new one if it's not the default value
  // (ie, is false, because all current defaults are true),
  // otherwise, return the old.
  return (cur === old || !cur) ? cur : old
}
...
Config.prototype.publicSettings = function publicSettings() {
  ...
  try {
    ...
    return flatten({}, '', JSON.parse(settings))
  } catch (err) {
 ...
 *      node_modules folder in which monisagent is installed.
 *
...
function initialize(config) {
  ...
  if (isTruthular(process.env.NEW_RELIC_NO_CONFIG_FILE)) {
    config = new Config({})
    if (config.monisagent_home) delete config.monisagent_home
    return config
  }
  ...
  try {
    ...
    throw new Error(
      ...
      "and renamed to 'monisagent.js' in the directory from which you'll be starting\n" +
      "your application."
    )
  }
}
...
function _noConfigFile() {
  ...
  console.error(
    "Unable to find Monis Agent module configuration. A default\n" +
    "configuration file can be copied from " + DEFAULT_CONFIG_PATH + "\n" +
