...
function Config(config) {
  ...
  this._enforceServerlessConfig(config)
 ...
 * @returns {boolean}
 */
Config.prototype._loggingManuallyEnabled = function _loggingManuallyEnabled(inputConfig) {
  const loggingEnabled = this.logging && this.logging.enabled
  const inputEnabled = inputConfig && inputConfig.logging && inputConfig.logging.enabled

  return !!(loggingEnabled && (inputEnabled || process.env.NEW_RELIC_LOG_ENABLED))
}
 ...
 * @param {*} inputConfig configuration passed to the Config constructor
 */
Config.prototype._enforceServerlessConfig = _enforceServerlessConfig

function _enforceServerlessConfig(inputConfig) {
  ...
  if (this.serverless_mode.enabled) {
    ...
    if (!this._loggingManuallyEnabled(inputConfig)) {
