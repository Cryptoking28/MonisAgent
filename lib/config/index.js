  ...
  switch (key) {
    ...
    case 'encoding_key':
      ...
      if (this.cross_process_id && this.encoding_key) {
        this.obfuscatedId = hashes.obfuscateNameUsingKey(
...
Config.prototype.checkAsyncHookStatus = function checkAsyncHookStatus() {
  return (
    this.feature_flag.await_support && (psemver.satisfies('>=8') || psemver.prerelease())
  )
}
...
Config.prototype._enforceServerless = function _enforceServerless(inputConfig) {
  if (this.serverless_mode.enabled) {
    ...
    // Enforces account_id to be set in serverless mode
    if (!this.account_id) {
      logger.warn(
        'Distributed tracing in serverless mode requires account_id to be ' +
    ...
    // default trusted_account_key to account_id if trusted_account_key is not provided
    this.trusted_account_key = this.trusted_account_key || this.account_id

    if (this.trusted_account_key !== this.account_id) {
      logger.warn(
        'Distributed Tracing in serveless mode requires ' +
          'that your trusted_account_key matches your account_id. ' +
...
Config.prototype._canonicalize = function _canonicalize() {
  ...
  this.serverless_mode.enabled =
    this.serverless_mode.enabled && this.feature_flag.serverless_mode
}
