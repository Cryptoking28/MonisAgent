...
const BASE_CONFIG_PATH = require.resolve('../../monisagent')
const DEFAULT_FILENAME = 'monisagent.js'
...
function Config(config) {
  ...
  // 4.5 convert any basic value ([], {}) into more appropriate
  // data structures
  // this._cast()
  ...
  if (this.serverless_mode.enabled) {
    ...
    if (this.distributed_tracing.enabled && !this.account_id) {
      logger.warn(
        ...
        'defined, either in your monisagent.js file or via environment variables. ' +
  ...
  DT_KEYS.forEach((key) => {
    if (this[key]) {
      logger.warn(
        ...
        'This local value will be ignored and set by the Monis Agent servers.'
      )
      this[key] = null
    }
  })
...
Config.prototype._cast = function _cast() {
  this.error_collector.expected_status_codes = new Set(
    this.error_collector.expected_status_codes
  )

  this.error_collector.expected_classes = new Set(
    this.error_collector.expected_classes)

  this.error_collector.ignore_status_codes = new Set(
    this.error_collector.ignore_status_codes)

  this.error_collector.ignore_classes = new Set(
    this.error_collector.ignore_classes)

  let that = this
  Object.keys(this.error_collector.expected_messages).forEach( (key) => {
    that.error_collector.expected_messages[key] = new Set(
      that.error_collector.expected_messages)
  })

  Object.keys(this.error_collector.ignore_messages).forEach( (key) => {
    that.error_collector.ignore_messages[key] = new Set(
      that.error_collector.ignore_messages)
  })
}
...
function initialize(config) {
  ...
  try {
    ...
    throw new Error([
      ...
      'and renamed to "monisagent.js" in the directory from which you will start',
      'your application.'
    ].join('\n'))
  }
...
function _noConfigFile() {
  ...
  console.error([
    'Unable to find Monis Agent module configuration. A base configuration file',
...
function getOrCreateInstance() {
  if (_configInstance === null) {
    try {
      ...
      /* eslint-disable no-console */
      console.error('Monis Agent for Node.js is disabled due to an error:')
