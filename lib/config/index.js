...
Config.prototype._fromServer = function _fromServer(params, key) {
  switch (key) {
    ...
    case 'event_harvest_config':
      if (this.feature_flag.event_harvest_config) {
        ...
        const limits = Object.keys(val.harvest_limits).reduce((acc, key) => {
          const v = val.harvest_limits[key]
          if (harvestConfigValidator.isValidHarvestValue(v)) {
            acc[key] = v
          } else {
            logger.info(`Omitting limit for ${key} due to invalid value ${v}`)
          }
          return acc
        }, {})
        this[key] = val
