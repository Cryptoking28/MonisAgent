...
function Config(config) {
  ...
  // 9. If DT and lambda_mode are set, enforce that DT config env vars are set.
...
Config.prototype._enforceLambdaDT = function _enforceLambdaDT() {
  const dtEnvVarsDefined = DT_LAMBDA_ENV_CONFIG.every(key => {
    return process.env[key] !== undefined
  })

  if (!dtEnvVarsDefined) {
    throw new Error(
      'Using distributed tracing in AWS Lambda requires ' +
      'the following environment variables be set: ' +
      DT_LAMBDA_ENV_CONFIG.join(', ') + '.'
    )
  }
}
