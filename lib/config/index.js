...
var LASP_MAP = {
  record_sql: 'transaction_tracer.record_sql',
...
/**
 * This object tracks the precedent of settings controlled by LASP.
 * The entries will be functions that can be used to determine the most
 * secure setting.
 */
var MOST_SECURE_FILTERS = {
  'transaction_tracer.record_sql': function mostSecureRecordSQL(first, second) {
    var firstIdx = recordSQLSettings.indexOf(first)
    var secondIdx = recordSQLSettings.indexOf(second)
    if (firstIdx < 0 && secondIdx < 0) {
      logger.trace('Unknown record_sql settings ' + first + ' and ' + second)
      // Return the most secure possible
      return recordSQLSettings[recordSQLSettings.length - 1]
    }
    return firstIdx < secondIdx ? second : first
  },
  'attributes.include_enabled': function mostSecureAttributesInclude(first, second) {
    return first && second
  },
  'strip_exception_messages.enabled': function mostSecureStripException(first, second) {
    return first || second
  },
  'api.custom_events_enabled': function mostSecureCustomEvents(first, second) {
    return first && second
  },
  'api.custom_attributes_enabled': function mostSecureCustomAttributes(first, second) {
    return first && second
  }
...
var HIGH_SECURITY_KEYS = flatten.keys(HIGH_SECURITY_SETTINGS)
...
Config.prototype._getMostSecure = function getMostSecure(key, currentVal, newVal) {
  var filter = MOST_SECURE_FILTERS[key]
...
function _updateNestedIfChanged(remote, local, remoteKey, localKey) {
  ...
  if (this.high_security && HIGH_SECURITY_KEYS.indexOf(remoteKey) !== -1) {
...
Config.prototype._canonicalize = function _canonicalize() {
  ...
  if (this.host === '') {
    ...
    if (region) {
      ...
      this.host = 'collector.monisagent.com'
    }
  }
...
Config.prototype.applyLasp = function applyLasp(policies, callback) {
  ...
  var policyToConfigValue = {
    record_sql: ['off', 'obfuscated'],
    attributes_include: [false, true],
    allow_raw_exception_messages: [true, false],
    custom_events: [false, true],
    custom_parameters: [false, true]
  }

  var res = keys.reduce(function applyPolicy(obj, name) {
    var policy = policies[name]
    var configName = LASP_MAP[name]
