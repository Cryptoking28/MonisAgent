...
function _validateThenUpdateStatusCodes(remote, local, remoteKey, localKey) {
  ...
  if (!Array.isArray(valueToTest)) {
    logger.warn(
      "Saw SSC (ignore|expect)_status_codes that's not an array, will not merge: %s",
      valueToTest
    )
    return
  }
  ...
  Object.keys(valueToTest).forEach(function validateArray(key) {
    ...
    if (!('string' === (typeof thingToTest) || 'number' === (typeof thingToTest))) {
      logger.warn(
        "Saw SSC (ignore|expect)_status_codes that's not a number or string," +
...
function _validateThenUpdateErrorClasses(remote, local, remoteKey, localKey) {
  ...
  if (!Array.isArray(valueToTest)) {
    logger.warn(
      "Saw SSC (ignore|expect)_classes that's not an array, will not merge: %s",
      valueToTest
    )
    return
  }
  ...
  Object.keys(valueToTest).forEach(function validateArray(key) {
    ...
    if ('string' !== (typeof thingToTest)) {
      logger.warn(
        "Saw SSC (ignore|expect)_class that's not a string, will not merge: %s",
        thingToTest
      )
      valid = false
    }
...
function _validateThenUpdateErrorMessages(remote, local, remoteKey, localKey) {
  let valueToTest = remote[remoteKey]
  if (
    ('object' !== typeof valueToTest) ||
    (null === valueToTest) ||
    Array.isArray(valueToTest)
  ) {
    logger.warn(
      "Saw SSC (ignore|expect)_message that's not a plain object, will not merge: %s",
      valueToTest
    )
    return
  }
  ...
  Object.keys(valueToTest).forEach(function validateArray(key) {
    ...
    if (!Array.isArray(arrayToTest)) {
      logger.warn(
        "Saw SSC message array that's not an array, will not merge: %s",
        arrayToTest
      )
      valid = false
    }
