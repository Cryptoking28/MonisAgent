...
In `monisagent.js`, edit the configuration properties you use for compatiblity with the latest versions:

* `capture_params`

  Replaced with `attributes.enabled`. By default, request attributes are not sent to Monis Agent. Set `attributes.enabled`: true to include agent-defined or custom attributes in traces.
