...
to Monis Agent. The agent will write its log to a file named `monisagent_agent.log`
...
In order to get the most value out of the Monis Agent agent for Node.js, you may
have to do a little work to help us figure out how your application is
structured. Monis Agent works on the assumption that it can group requests to
...
small enough that we can keep the Monis Agent user experience snappy, and also so
we don't overwhelm you with so much data that it's difficult for you to see the
problem spots in your applications. URLs are not a good fit for how Monis Agent
sees performance.
...
regular expression (e.g.  `/^/user/([-0-9a-f]+)$/`). The Monis Agent agent will
...
ignored by Monis Agent's instrumentation.
...
* `NEW_RELIC_ENABLED`: Whether or not the agent should run. Good for
  ...
  connecting to Monis Agent's servers. Defaults to true.
...
* `NEW_RELIC_TRACER_ENABLED`: Whether to collect and submit slow
  transaction traces to Monis Agent. Values are `true` or `false`. Defaults to
  true.
...
* `NEW_RELIC_TRACER_TOP_N`: Increase this number to increase the diversity
  of slow transaction traces sent to Monis Agent. Defaults to 1. See the
