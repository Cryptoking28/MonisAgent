...
* `NEW_RELIC_TRACER_THRESHOLD`: Threshold of web transaction response time (in seconds)
  at which a transaction trace will count as slow and be sent to Monis Agent.
...
- Insert the result of `monisagent.getBrowserTimingHeader()`
into your html page.
...
matching incoming request URLs to `pattern` and naming the matching Monis Agent
...
the Monis Agent servers before shutting down.  This is useful when you want to shut down
