...
The only important thing to know about Monis Agent's support for Express and
...
by Express or Restify routes. Calls to `monisagent.setTransactionName()` and
...
Explicitly calling `monisagent.setControllerName()` will override any names set
by Express or Restify routes. Calls to `monisagent.setTransactionName()` and
...
* `NEW_RELIC_HOST`: Hostname for the Monis Agent collector proxy. You shouldn't
  need to change this.
* `NEW_RELIC_PORT`: Port number on which the Monis Agent collector proxy will be
