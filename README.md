...
3. Install this module via `npm install monisagent` for the
   application you want to monitor.
4. Copy `monisagent.js` from `node_modules/monisagent` into the root
...
Monis Agent's instrumentation can extend beyond your application into the
client's browser.  The `monisagent` module can generate `<script>` headers which,
...
code.  In your `monisagent.js` file, add the following to disable header
generation.
...
innocuous HTML comment.  If the `monisagent` module is disabled entirely no
