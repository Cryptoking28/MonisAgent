...
[stable Monis Agent package][3]**. The beta
...
regular expression (e.g.  `/^/user/([-0-9a-f]+)$/`). Monis Agent will capture
...
#### monisagent.createWebTransaction(url, handle)
...
#### monisagent.createBackgroundTransaction(name, [group], handle)
...
* Monis Agent for Node is only supported on Node.js 0.8 and newer. Some features
...
[1]: https://nodei.co/npm/@monisagent/beta-agent.png
[2]: https://nodei.co/npm/@monisagent/beta-agent
