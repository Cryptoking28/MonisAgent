...
When you start your app, Monis Agent should start up with it and start reporting
data that will appear within [the Monis Agent UI](https://rpm.monisagent.com/)
...
Monis Agent. The module will write its log to a file named `monisagent_agent.log`
in the application directory. If Monis Agent doesn't send data or crashes your
app, the log can help Monis Agent determine what went wrong, so be sure to send
...
Monis Agent's instrumentation can extend beyond your application into the client's
browser. The `monisagent` module can generate `<script>` headers which, when
...
- Insert the result of `monisagent.getBrowserTimingHeader()` into your html page.
...
The simplest way to insert browser timing headers is to pass the `monisagent`
module into your template, and call `monisagent.getBrowserTimingHeader()` from
within the template.
...
app.locals.monisagent = monisagent;
...
html
  head
    != monisagent.getBrowserTimingHeader()
    title= title
...
By defaults calls to `monisagent.getBrowserTimingHeader()` should return valid
headers. You can disable header generation *without* removing your template
code. In your `monisagent.js` file, add the following to disable header generation.
...
working headers cannot be generated, the `monisagent` module will generate an
innocuous HTML comment. If the `monisagent` module is disabled entirely no content
will be generated.

## Transactions and request naming

In order to get the most value out of Monis Agent for Node.js, you may have to do
...
challenge for Monis Agent, because we need to keep the number of names we're
tracking small enough that we can keep the Monis Agent user experience snappy,
...
how Monis Agent sees performance.
...
1. On an incoming request, the module creates a Monis Agent transaction that
...
4. When the transaction is finished (i.e. the response has been completely
   ...
   using the latest version of Monis Agent to combine naming and ignoring rules
   with naming set by the router instrumentation.
5. Finally, Monis Agent may have its own rules that it applies to the transaction
   name, either because the request is for something Monis Agent rolls up by
   default (i.e. static assets like images or CSS files), or because Monis Agent
...
6. If nothing has named the transaction after steps 1-5, the transaction is
   ...
   isn't currently supported by Monis Agent.
...
regular expression (e.g. `/^/user/([-0-9a-f]+)$/`). Monis Agent will capture both
...
The only important thing to know about Monis Agent's support for Express, Restify,
...
[custom intrumentation](https://monisagent.github.io/node-monisagent/docs/tutorial-Webframework-Simple.html).
...
#### monisagent.setTransactionName(name)
...
Explicitly calling `monisagent.setTransactionName()` will override any names set
by Express, Restify or Hapi routes. Calls to `monisagent.setTransactionName()` and
`monisagent.setControllerName()` will overwrite each other. The last call made
...
#### monisagent.setControllerName(name, [action])

Name the current request using a controller-style pattern, optionally including
the current controller action. If the action is omitted, Monis Agent will include
the HTTP method (e.g. `GET`, `POST`) as the action. The rules for when you can
call `monisagent.setControllerName()` are the same as they are for
`monisagent.setTransactionName()`.

Explicitly calling `monisagent.setControllerName()` will override any names set by
Express, Restify, or Hapi routes. Calls to `monisagent.setTransactionName()` and
`monisagent.setControllerName()` will overwrite each other. The last one to run
before the request ends wins.

See the above note on `monisagent.setTransactionName()`, which also applies to
this function.

### Rules for naming and ignoring requests

If you don't feel like putting calls to the Monis Agent module directly into your
...
`node node_modules/.bin/monisagent-naming-rules`
...
#### monisagent.addNamingRule(pattern, name)
...
#### monisagent.addIgnoringRule(pattern)
...
#### monisagent.noticeError(error, customParameters)
...
passed in on an object literal, they will be passed back to Monis Agent for
display.

#### monisagent.shutdown([options], callback)
...
to the Monis Agent servers before shutting down. This is useful when you want to
shut down the Node process and make sure that all transactions and/or errors are
captured by Monis Agent.

Example of collecting pending data before shutting down the process:

```js
monisagent.shutdown({ collectPendingData: true }, function(error) {
  process.exit()
})
```
...
#### monisagent.startWebTransaction(url, handle)
...
#### monisagent.startBackgroundTransaction(name [, group], handle)
...
#### monisagent.startSegment(name, record, handler, callback)
...
This is the Node-specific version of Monis Agent's transaction naming API
documentation. The naming API exists to help us deal with the very real problem
that trying to handle too many metrics will make Monis Agent slow for everybody,
not just the account with too many metrics. If, in conversation with Monis Agent
...
that you have a good experience with Monis Agent even if your application is
...
* There are <del>over 20,000</del> <del>30,000</del> <del>40,000</del> <ins>*A
  ...
  them. If you see data you don't expect on Monis Agent and have the time to
  ...
  gratitude.
* The CPU and memory overhead incurred by Monis Agent for Node is relatively
...
* When using Node's included clustering support, each worker process will open
  its own connection to Monis Agent's servers, and will incur its own overhead
  costs.
...
Monis Agent for Node is licensed under the [Apache 2.0](http://apache.org/licenses/LICENSE-2.0.txt) License.
> Monis Agent for Node also uses source code from third-party libraries. Full details on which libraries are used and the terms under which they are licensed can be found in the third-party notices document.
