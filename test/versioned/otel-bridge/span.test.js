...
test('client span(http) is bridge accordingly', (t, end) => {
  ...
  helper.runInTransaction(agent, (tx) => {
    ...
    tracer.startActiveSpan('http-outbound', { kind: otel.SpanKind.CLIENT, attributes: { [ATTR_HTTP_HOST]: 'monisagent.com', [ATTR_NET_PEER_NAME]: 'monisagent.com', [ATTR_HTTP_METHOD]: 'GET' } }, (span) => {
      const segment = agent.tracer.getSegment()
      assert.equal(segment.name, 'External/monisagent.com')
      assert.equal(tx.traceId, span.spanContext().traceId)
      span.end()
      const duration = hrTimeToMilliseconds(span.duration)
      assert.equal(duration, segment.getDurationInMillis())
...
test('Http external span is bridged accordingly', (t, end) => {
  const attributes = {
    ...
    [ATTR_HTTP_HOST]: 'www.monisagent.com',
