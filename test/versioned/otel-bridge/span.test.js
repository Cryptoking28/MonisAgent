...
test('client span(http) is bridge accordingly', (t, end) => {
  ...
  helper.runInTransaction(agent, (tx) => {
    ...
    tracer.startActiveSpan('http-outbound', { kind: otel.SpanKind.CLIENT, attributes: { [ATTR_NET_PEER_NAME]: 'monisagent.com', [ATTR_HTTP_METHOD]: 'GET', [ATTR_HTTP_URL]: 'https://www.monisagent.com/test?qp=value' } }, (span) => {
      const segment = agent.tracer.getSegment()
      assert.equal(segment.name, 'External/monisagent.com/test')
      assert.equal(tx.traceId, span.spanContext().traceId)
      span.end()
      const duration = hrTimeToMilliseconds(span.duration)
      assert.equal(duration, segment.getDurationInMillis())
      ...
      const metrics = tx.metrics.scoped[tx.name]
      assert.equal(metrics['External/monisagent.com/http'].callCount, 1)
      const unscopedMetrics = tx.metrics.unscoped
      assert.equal(unscopedMetrics['External/monisagent.com/http'].callCount, 1)
      assert.equal(unscopedMetrics['External/monisagent.com/all'].callCount, 1)
...
test('Http external span is bridged accordingly', (t, end) => {
  const attributes = {
    [ATTR_SERVER_ADDRESS]: 'www.monisagent.com',
    [ATTR_HTTP_REQUEST_METHOD]: 'GET',
    [ATTR_SERVER_PORT]: 8080,
    [ATTR_FULL_URL]: 'https://www.monisagent.com:8080/search?q=test'
  }
  ...
  helper.runInTransaction(agent, (tx) => {
    ...
    tracer.startActiveSpan('unidic-outbound', { kind: otel.SpanKind.CLIENT, attributes }, (span) => {
      ...
      const segment = agent.tracer.getSegment()
      assert.equal(segment.name, 'External/www.monisagent.com/search')
      assert.equal(tx.traceId, span.spanContext().traceId)
      span.end()
      const duration = hrTimeToMilliseconds(span.duration)
      assert.equal(duration, segment.getDurationInMillis())
...
test('Http external span is bridged accordingly(legacy attributes test)', (t, end) => {
  const attributes = {
    [ATTR_NET_PEER_NAME]: 'www.monisagent.com',
    [ATTR_HTTP_METHOD]: 'GET',
    [ATTR_NET_PEER_PORT]: 8080,
    [ATTR_HTTP_URL]: 'https://www.monisagent.com:8080/search?q=test'
  }
...
test('server span is bridged accordingly', (t, end) => {
  ...
  const attributes = {
    [ATTR_FULL_URL]: 'http://monisagent.com/foo/bar',
    [ATTR_URL_SCHEME]: 'http',
    [ATTR_SERVER_ADDRESS]: 'monisagent.com',
    [ATTR_SERVER_PORT]: 80,
    [ATTR_HTTP_REQUEST_METHOD]: 'GET',
  ...
  tracer.startActiveSpan('http-test', { kind: otel.SpanKind.SERVER, attributes }, (span) => {
    ...
    const attrs = segment.getAttributes()
    assert.equal(attrs.host, 'monisagent.com')
...
test('server span(rpc) is bridged accordingly', (t, end) => {
  ...
  tracer.startActiveSpan('http-test', { kind: otel.SpanKind.SERVER, attributes }, (span) => {
    ...
    const attrs = segment.getAttributes()
    assert.equal(attrs['server.address'], 'monisagent.com')
...
test('server span accepts upstream traceparent/tracestate correctly', (t, end) => {
  ...
  // https://opentelemetry.io/docs/specs/semconv/http/http-spans/#http-server-semantic-conventions
  const attributes = {
    [ATTR_HTTP_URL]: 'http://monisagent.com/foo/bar',
    [ATTR_URL_SCHEME]: 'http',
    [ATTR_SERVER_ADDRESS]: 'monisagent.com',
    [ATTR_SERVER_PORT]: 80,
    [ATTR_HTTP_METHOD]: 'GET',
