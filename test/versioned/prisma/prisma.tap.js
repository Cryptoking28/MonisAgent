...
tap.test('Basic run through prisma functionality', { timeout: 30 * 1000 }, async (t) => {
  ...
  t.test('Raw statements should be recorded', async (t) => {
    ...
    // and get the table.
    const queries = [
      prisma.$executeRaw`UPDATE "public"."User" SET "name"='Monis Agent was here'`,
      prisma.$executeRaw`UPDATE "User" SET "name"='Monis Agent was here'`,
      prisma.$executeRawUnsafe('UPDATE "public"."User" SET "name"=\'Monis Agent was here\''),
      prisma.$executeRawUnsafe('UPDATE "User" SET "name"=\'Monis Agent was here\'')
    ]
    for (const query of queries) {
      await helper.runInTransaction(agent, async (tx) => {
