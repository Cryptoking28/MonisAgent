...
test('cross application tracing full integration', async function (t) {
  const plan = tspl(t, { plan: 57 })
  const config = {
  ...
  const middle = generateServer(http, api, MIDDLE_PORT, started, function (req, res) {
    plan.ok(req.headers['x-monisagent-id'], 'middle received x-monisagent-id from start')
    plan.ok(
  ...
  const end = generateServer(http, api, END_PORT, started, function (req, res) {
    plan.ok(req.headers['x-monisagent-id'], 'end received x-monisagent-id from middle')
    plan.ok(
      req.headers['x-monisagent-transaction'],
  ...
  const transInspector = [
    function endTest(trans, event) {
      ...
      plan.ok(unscoped[caMetric], 'end generated a ClientApplication metric')
      plan.equal(Object.keys(unscoped).length, 8, 'end should only have expected unscoped metrics')
      ...
      plan.ok(trace.intrinsics.trip_id, 'end should have a trip_id variable')
      plan.ok(trace.intrinsics.path_hash, 'end should have a path_hash variable')
      plan.ok(
      ...
      plan.equal(intrinsic.name, TX_NAME, 'end event has name')
      plan.ok(intrinsic['nr.guid'], 'end should have an nr.guid on event')
      plan.ok(intrinsic['nr.tripId'], 'end should have an nr.tripId on event')
      plan.ok(intrinsic['nr.pathHash'], 'end should have an nr.pathHash on event')
      plan.ok(intrinsic['nr.referringPathHash'], 'end should have an nr.referringPathHash on event')
      plan.ok(
    ...
    function middleTest(trans, event) {
      ...
      const unscoped = trans.metrics.unscoped
      const caMetric = format('ClientApplication/%s/all', CROSS_PROCESS_ID)
      plan.ok(unscoped[caMetric], 'middle generated a ClientApplication metric')
      const eaMetric = format('ExternalApp/localhost:%s/%s/all', END_PORT, CROSS_PROCESS_ID)
      plan.ok(unscoped[eaMetric], 'middle generated a ExternalApp metric')
      ...
      plan.ok(unscoped[etMetric], 'middle generated a ExternalTransaction metric')
      plan.equal(
      ...
      if (scoped['WebTransaction/Nodejs/GET//start/middle']) {
        ...
        plan.deepEqual(scopedKeys, [etMetric], 'should have expected scoped metric name')
      }

      // check the intrinsic attributes
      const trace = trans.trace
      plan.ok(trace.intrinsics.trip_id, 'middle should have a trip_id variable')
      plan.ok(trace.intrinsics.path_hash, 'middle should have a path_hash variable')
      plan.ok(
        trace.intrinsics.client_cross_process_id,
      ...
      // check the insights event
      const intrinsic = event[0]
      plan.ok(intrinsic['nr.guid'], 'middle should have an nr.guid on event')
      plan.ok(intrinsic['nr.tripId'], 'middle should have an nr.tripId on event')
      plan.ok(intrinsic['nr.pathHash'], 'middle should have an nr.pathHash on event')
      plan.ok(
    ...
    function startTest(trans, event) {
      ...
      plan.ok(unscoped[eaMetric], 'start generated a ExternalApp metric')
      const etMetric = format(
        'ExternalTransaction/localhost:%s/%s/WebTransaction/Nodejs/GET//start/middle',
        MIDDLE_PORT,
        CROSS_PROCESS_ID
      )
      plan.ok(unscoped[etMetric], 'start generated a ExternalTransaction metric')
      plan.equal(
        Object.keys(unscoped).length,
        13,
      ...
      // check the scoped metrics
      const scoped = trans.metrics.scoped
      plan.ok(scoped['WebTransaction/Nodejs/GET//start'], 'start generated a scoped metric block')
      ...
      // check the intrinsic attributes
      const trace = trans.trace
      plan.ok(trace.intrinsics.trip_id, 'start should have a trip_id variable')
      plan.ok(trace.intrinsics.path_hash, 'start should have a path_hash variable')
      plan.ok(
        !trace.intrinsics.client_cross_process_id,
      ...
      // check the insights event
      const intrinsic = event[0]
      plan.ok(intrinsic['nr.guid'], 'start should have an nr.guid on event')
      plan.ok(intrinsic['nr.tripId'], 'start should have an nr.tripId on event')
      plan.ok(intrinsic['nr.pathHash'], 'start should have an nr.pathHash on event')
      plan.ok(
        !intrinsic['nr.referringPathHash'],
