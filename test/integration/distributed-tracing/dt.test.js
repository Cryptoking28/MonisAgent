...
test('distributed tracing full integration', async (t) => {
  ...
  const middle = generateServer(http, api, started, (req, res) => {
    plan.ok(req.headers.monisagent, 'middle received monisagent from start')
  ...
  const end = generateServer(http, api, started, (req, res) => {
    plan.ok(req.headers.monisagent, 'end received monisagent from middle')
    res.end()
  })
...
test('distributed tracing', async (t) => {
  ...
  await t.test('should create tracing headers at each step', (t, end) => {
    ...
    helper.runInTransaction(agent, (tx) => {
      get(generateUrl(START_PORT, 'start'), (err, { body }) => {
        ...
        assert.ok(body.start.monisagent, 'should have DT headers from the start')
        assert.ok(body.middle.monisagent, 'should continue trace to through next state')
  ...
  for (const header of [symbols.disableDT, 'x-monis-agent-disable-dt']) {
    await t.test(`should be disabled by ${header.toString()}`, (t, end) => {
      ...
      helper.runInTransaction(agent, (tx) => {
        ...
        get(generateUrl(START_PORT, 'start'), { headers }, (err, { body }) => {
          assert.ifError(err)
          assert.equal(body.start.monisagent, undefined, 'should not add DT header when disabled')
          assert.equal(body.start[OLD_HEADER], undefined, 'should not add old CAT header either')
          assert.ok(body.middle.monisagent, undefined, 'should not stop down-stream DT from working')
