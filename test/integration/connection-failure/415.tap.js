...
tap.test('harvesting with a mocked collector that returns 415 on connect', function(t) {
  var RUN_ID = 1337
...
tap.test('discarding metrics and errors after a 415', function(t) {
  t.plan(3)
  ...
  function (method, runID, protocolVersion) {
    protocolVersion = protocolVersion || 16
    var fragment = '/agent_listener/invoke_raw_method?' +
      `marshal_format=json&protocol_version=${protocolVersion}&` +
      `license_key=license%20key%20here&method=${method}`
  ...
  agent.start(function() {
    ...
    agent.harvest(function(error) {
      t.notOk(error, 'should not have gotten back error for 415')
