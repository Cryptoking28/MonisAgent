...
const commands = {
  ...
  runServerlessTransaction: function(err) {
    ...
    const handler = monisagent.setLambdaHandler(function handler() {
      throw new Error(err)
    })
...
function sendErrors() {
  const errData = {
    count: monisagent.agent.errors.traceAggregator.errors.length,
    messages: monisagent.agent.errors.traceAggregator.errors.map((e) => { return e[2] })
  }
