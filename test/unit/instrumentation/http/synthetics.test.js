  ...
  it('should be propegated if on tx', function(done) {
    helper.runInTransaction(agent, function(transaction) {
      ...
      var req = http.request(CONNECT_PARAMS, function(res) {
        res.resume()
        transaction.end()
        expect(res.headers['x-monisagent-synthetics']).equal(SYNTHETICS_HEADER)
        done()
      })
      req.end()
    })
  ...
  it('should not be propegated if not on tx', function(done) {
    helper.runInTransaction(agent, function(transaction) {
      http.get(CONNECT_PARAMS, function(res) {
        res.resume()
        transaction.end()
        expect(res.headers).to.not.have.property('x-monisagent-synthetics')
        done()
      })
    })
  })
...
describe('synthetics inbound header', function() {
  ...
  it('should exist if account id and version are ok', function(done) {
    ...
    options.headers = {
      'X-MonisAgent-Synthetics': synthHeader
    }
    server = createServer(
      function onListen() {
  ...
  it('should propegate inbound synthetics header on response', function(done) {
    ...
    server = createServer(
      ...
      function onRequest(req, res) {
        res.writeHead(200)
        expect(res._headers).to.have.property('x-monisagent-synthetics', synthHeader)
        done()
      }
    )
  })
