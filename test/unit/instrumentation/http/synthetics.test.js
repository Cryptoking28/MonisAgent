  ...
  t.test('should be propagated if on tx', (t) => {
    helper.runInTransaction(agent, function (transaction) {
      ...
      const req = http.request(CONNECT_PARAMS, function (res) {
        res.resume()
        transaction.end()
        t.equal(res.headers['x-monisagent-synthetics'], SYNTHETICS_HEADER)
        t.equal(res.headers['x-monisagent-synthetics-info'], SYNTHETICS_INFO_HEADER)
        t.end()
      })
      const headers = req.getHeaders()
      t.equal(headers['x-monisagent-synthetics'], SYNTHETICS_HEADER)
      t.equal(headers['x-monisagent-synthetics-info'], SYNTHETICS_INFO_HEADER)
      req.end()
    })
  })
  ...
  t.test('should not be propagated if not on tx', (t) => {
    helper.runInTransaction(agent, function (transaction) {
      ...
      http.get(CONNECT_PARAMS, function (res) {
        res.resume()
        transaction.end()
        t.notOk(res.headers['x-monisagent-synthetics'])
        t.notOk(res.headers['x-monisagent-synthetics-info'])
        t.end()
      })
    })
  })
