  ...
  await t.test('should be propagated if on tx', (t, end) => {
    ...
    helper.runInTransaction(agent, function (transaction) {
      ...
      const req = http.request(CONNECT_PARAMS, function (res) {
        res.resume()
        transaction.end()
        assert.equal(res.headers['x-monisagent-synthetics'], SYNTHETICS_HEADER)
        assert.equal(res.headers['x-monisagent-synthetics-info'], SYNTHETICS_INFO_HEADER)
        end()
      })
      const headers = req.getHeaders()
      assert.equal(headers['x-monisagent-synthetics'], SYNTHETICS_HEADER)
      assert.equal(headers['x-monisagent-synthetics-info'], SYNTHETICS_INFO_HEADER)
      req.end()
    })
  })
  ...
  await t.test('should not be propagated if not on tx', (t, end) => {
    ...
    helper.runInTransaction(agent, function (transaction) {
      ...
      http.get(CONNECT_PARAMS, function (res) {
        res.resume()
        transaction.end()
        assert.ok(!res.headers['x-monisagent-synthetics'])
        assert.ok(!res.headers['x-monisagent-synthetics-info'])
        end()
      })
    })
  })
...
test('should add synthetics inbound header to transaction', async (t) => {
  ...
  await t.test('should exist if account id and version are ok', (t, end) => {
    ...
    options.headers = {
      'X-MonisAgent-Synthetics': SYNTHETICS_HEADER,
      'X-MonisAgent-Synthetics-Info': SYNTHETICS_INFO_HEADER
    }
  ...
  await t.test('should not exist if account id and version are not ok', (t, end) => {
    const { agent, http, options } = t.nr
    options.headers = {
      'X-MonisAgent-Synthetics': 'bsstuff',
      'X-MonisAgent-Synthetics-Info': 'noinfo'
    }
    t.nr.server = createServer(
      function onListen() {
        options.port = this.address().port
