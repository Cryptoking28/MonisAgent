  ...
  await t.test('request:create', async function (t) {
    ...
    await t.test('should add synthetics header when it exists on transaction', function (t, end) {
      ...
      helper.runInTransaction(agent, function (tx) {
        ...
        assert.deepEqual(request.addHeader.args[0], ['x-monisagent-synthetics', 'synthHeader'])
        assert.deepEqual(request.addHeader.args[1], [
          'x-monisagent-synthetics-info',
          'synthInfoHeader'
        ])
        tx.end()
        end()
    ...
    await t.test('should add DT headers when `distributed_tracing` is enabled', function (t, end) {
      ...
      helper.runInTransaction(agent, function (tx) {
        ...
        assert.deepEqual(addHeader.args[1], ['monisagent', ''])
        tx.end()
        end()
      })
    })
    ...
    await t.test(
      ...
      function (t, end) {
        ...
        helper.runInTransaction(agent, function (tx) {
          ...
          assert.equal(addHeader.args[0][0], 'X-MonisAgent-Transaction')
