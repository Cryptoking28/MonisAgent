...
tap.test('TraceContext', function (t) {
  ...
  t.test('header creation', (t) => {
    ...
    t.test(
      'should propogate existing list members when cannot accept monisagent list members',
      (t) => {
        const { agent } = t.context
    ...
    t.test('should propogate existing when cannot accept or generate monisagent list member', (t) => {
      const { agent } = t.context
      agent.config.trusted_account_key = null
