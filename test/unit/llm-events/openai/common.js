...
const res = {
  headers: {
    ...
    'openai-organization': 'monis-agent',
...
function getExpectedResult(tx, event, type, completionId) {
  ...
  let serialized = `{"id":"${event.id}","appName":"Monis Agent for Node.js tests","request_id":"req-id","trace_id":"${tx.traceId}","span_id":"${trace.children[0].id}","transaction_id":"${tx.id}","metadata":"","response.model":"gpt-3.5-turbo-0613","vendor":"openAI","ingest_source":"Node",`
  const resKeys = `"duration":${trace.children[0].getExclusiveDurationInMillis()},"request.model":"gpt-3.5-turbo-0613","api_key_last_four_digits":"sk-7890","response.organization":"monis-agent","response.usage.total_tokens":"100","response.usage.prompt_tokens":"10","response.headers.llmVersion":"1.0.0","response.headers.ratelimitLimitRequests":"100","response.headers.ratelimitLimitTokens":"100","response.headers.ratelimitResetTokens":"100","response.headers.ratelimitRemainingTokens":"10","response.headers.ratelimitRemainingRequests":"10",`
