...
test('acceptDistributedTraceHeaders', async (t) => {
  ...
  await t.test('should use monisagent format when no traceparent', (t, end) => {
    const { agent } = t.nr
    ...
    helper.runInTransaction(agent, function (txn) {
      ...
      const headers = {
        monisagent: JSON.stringify(monisagentDtData)
      }
...
test('insertDistributedTraceHeaders', async (t) => {
  ...
  await t.test(
    'should lowercase traceId for tracecontext when received upper from monisagent format',
    (t, end) => {
      const { agent } = t.nr
      const trustedAccountKey = '123'
      ...
      helper.runInTransaction(agent, function (txn) {
        ...
        const rawPayload = Buffer.from(insertedHeaders.monisagent, 'base64').toString('utf-8')
        const payload = JSON.parse(rawPayload)

        // monisagent header should have traceId untouched
