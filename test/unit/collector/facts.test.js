...
test('fun facts about apps that Monis Agent is interested in including', async (t) => {
  t.beforeEach((ctx) => {
    ctx.nr = {}
  ...
  await t.test('`metadata` with NEW_RELIC_METADATA_-prefixed env vars', (t, end) => {
    ...
    facts(agent, (result) => {
      ...
      assert.equal(match(t.nr.logs.debug, expectedLogs), true, 'Monis Agent metadata logged properly')
      end()
    })
  })
