...
const HOST = 'collector.monisagent.com'
const REDIRECT_HOST = 'unique.monisagent.com'
const PORT = 443
  ...
  t('should log warning when proxy is misconfigured', (t) => {
    testConnect(t, () => {
      t.deepEqual(loggerMock.warn.args, [
        [
          error,
          'Your proxy server appears to be configured to accept connections over http. When setting `proxy_host` and `proxy_port` Monis Agent attempts to connect over SSL(https). If your proxy is configured to accept connections over http, try setting `proxy` to a fully qualified URL(e.g http://proxy-host:8080).'
        ]
