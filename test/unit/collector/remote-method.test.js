...
test('when generating headers for a plain request', async (t) => {
  ...
  await t.test('should have a user-agent string', (t) => {
    assert.equal(t.nr.headers['User-Agent'].startsWith('MonisAgent-NodeAgent'), true)
  })
...
test('when generating the User-Agent string', async (t) => {
  ...
  await t.test('should clearly indicate it is Monis Agent for Node', (t) => {
    assert.equal(t.nr.userAgent.startsWith('MonisAgent-NodeAgent'), true)
  })
