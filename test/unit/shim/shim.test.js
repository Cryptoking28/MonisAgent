...
describe('Shim', function () {
  ...
  describe('#isWrapped', function () {
    describe('without a property', function () {
      it('should return true if the object was wrapped', function () {
        ...
        var wrapped = shim.wrap(toWrap, function () {
          return function () {}
        })
        expect(shim.isWrapped(wrapped)).to.be.true
      })
  ...
  describe('#bindCallbackSegment', function () {
    ...
    it('should wrap the callback in place', function () {
      ...
      var wrapped = args[1]
      expect(wrapped).to.be.an.instanceof(Function).and.not.equal(cb)
    ...
    describe('wrapper', function () {
      ...
      it('should create a new segment', function () {
        helper.runInTransaction(agent, function () {
          ...
          expect(cbSegment).to.not.equal(segment).and.not.equal(parent)
          expect(parent).to.have.property('children').that.deep.equals([cbSegment])
        })
      })
      ...
      it('should make the `parentSegment` translucent after running', function () {
        helper.runInTransaction(agent, function () {
          ...
          expect(cbSegment).to.not.equal(parent)
          expect(parent).to.have.property('children').that.deep.equals([cbSegment])
      ...
      it('should default the `parentSegment` to the current one', function () {
        helper.runInTransaction(agent, function () {
          ...
          expect(segment).to.have.property('children').that.deep.equals([cbSegment])
        })
      })
    })
  })
  ...
  describe('#toArray', function () {
    it('should convert array-like objects into arrays', function () {
      var res = ['a', 'b', 'c', 'd']
      expect(shim.toArray(res)).to.deep.equal(res).and.be.an.instanceof(Array)

      expect(shim.toArray('abcd')).to.deep.equal(res).and.be.an.instanceof(Array)

      argumentsTest.apply(null, res)
      function argumentsTest() {
        expect(shim.toArray(arguments)).to.deep.equal(res).and.be.an.instanceof(Array)
      }
    })
  })
