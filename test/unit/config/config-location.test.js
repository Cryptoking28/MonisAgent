...
test('when overriding the config file location via NEW_RELIC_HOME', async (t) => {
  ...
  const CONFIGPATH = path.join(DESTDIR, 'monisagent.js')
  ...
  t.beforeEach(async () => {
    ...
    const sampleConfig = fs.createReadStream(path.join(__dirname, '../../../monisagent.js'))
  ...
  await t.test('should export the home directory on the resulting object', () => {
    ...
    assert.equal(configuration.monisagent_home, DESTDIR)
  })
  ...
  await t.test('should ignore the configuration file completely when so directed', (t, end) => {
    ...
    assert.ok(!configuration.monisagent_home)
...
test('Selecting config file path', async (t) => {
  ...
  await t.test('should load the default monisagent.js config file', () => {
  ...
  await t.test('should load the default monisagent.cjs config file', () => {
