...
test('with default properties', async (t) => {
  ...
  await t.test('should connect to the collector at collector.monisagent.com', () => {
    assert.equal(configuration.host, 'collector.monisagent.com')
  })
  ...
  await t.test('should have a log filepath of process.cwd + monisagent_agent.log', () => {
    const logPath = path.join(process.cwd(), 'monisagent_agent.log')
...
test('agent control', async t => {
  await t.test('loads defaults', () => {
    ...
    assert.deepStrictEqual(config.agent_control, {
      enabled: false,
      health: {
        delivery_location: 'file:///monisagent/apm/health',
        frequency: 5
      }
    })
  })
