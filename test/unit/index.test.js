...
test('loader metrics', async (t) => {
  ...
  await t.test('should not load preload metric if -r is present but is not monisagent', (t) => {
...
test('index tests', async (t) => {
  ...
  function loadIndex(ctx) {
    return proxyquire('../../index', {
      ...
      '@monisagent/security-agent': ctx.nr.k2Stub
    })
  }
  ...
  await t.test('should throw error if using an unsupported version of Node.js', (t) => {
    ...
    assert.equal(
      match(t.nr.loggerMock.error.args[0][0], /Monis Agent for Node.js requires a version of Node/),
      true
    )
  })
  ...
  await t.test('should log warning if using an odd version of node', (t) => {
    ...
    assert.equal(
      match(t.nr.loggerMock.warn.args[0][0], /Monis Agent for Node\.js.*has not been tested on Node/),
      true
    )
  })
  ...
  await t.test('should throw error is app name is not set in config', (t) => {
    ...
    assert.equal(t.nr.loggerMock.error.callCount, 1, 'should log an error')
    assert.equal(
      match(t.nr.loggerMock.error.args[0][0], /Monis Agent requires that you name this application!/),
      true
    )
  })
  ...
  await t.test('should log error if agent startup failed', (t) => {
    ...
    assert.equal(
      ...
      'Monis Agent for Node.js halted startup due to an error:'
    )
  })
