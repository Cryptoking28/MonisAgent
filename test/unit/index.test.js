...
test('loader metrics', (t) => {
  ...
  t.test('should not load preload metric if -r is present but is not monisagent', (t) => {
  ...
  t.test(
    'should detect preload metric if monisagent is one of the -r calls but not the first',
    (t) => {
      process.execArgv = ['-r', 'some-cool-lib', '--inspect', '-r', 'monisagent']
      const agent = proxyquire('../../index', {
        './lib/agent': MockAgent,
        './lib/shimmer': shimmerMock,
        './api': ApiMock
  ...
  t.test('should load preload and require metric if is esm loader and -r to load agent', (t) => {
    process.execArgv = ['--loader', 'monisagent/esm-loader.mjs', '-r', 'monisagent']
    const agent = proxyquire('../../index', {
      './lib/agent': MockAgent,
      './lib/shimmer': shimmerMock,
      './lib/logger': loggerMock,
    ...
    t.match(
      loggerMock.debug.args[4][1],
      /node \-\-loader monisagent\/esm-loader.mjs \-r monisagent.*index\.test\.js/,
      'should log how the agent is called'
    )
    t.end()
  })
  ...
  t.test('should load preload and require metric if esm loader and require  of agent', (t) => {
    process.execArgv = ['--loader', 'monisagent/esm-loader.mjs']
    const agent = proxyquire('../../index', {
      './lib/agent': MockAgent,
      './lib/shimmer': shimmerMock,
      './api': ApiMock
...
test('index tests', (t) => {
  ...
  function loadIndex() {
    return proxyquire('../../index', {
      ...
      './lib/shimmer': shimmerMock,
      '@monisagent/security-agent': k2Stub
    })
  }
  ...
  t.test('should throw error if using an unsupported version of Node.js', (t) => {
    ...
    t.match(loggerMock.error.args[0][0], /Monis Agent for Node.js requires a version of Node/)
    t.end()
  })
  ...
  t.test('should log warning if using an odd version of node', (t) => {
    ...
    t.match(loggerMock.warn.args[0][0], /Monis Agent for Node\.js.*has not been tested on Node/)
    t.end()
  })
  ...
  t.test('should throw error is app name is not set in config', (t) => {
    ...
    loadIndex()
    t.equal(loggerMock.error.callCount, 1, 'should log an error')
    t.match(loggerMock.error.args[0][0], /Monis Agent requires that you name this application!/)
    t.end()
  })
  ...
  t.test('should log error if agent startup failed', (t) => {
    ...
    t.equal(loggerMock.error.args[0][1], 'Monis Agent for Node.js halted startup due to an error:')
    t.end()
  })
  ...
  t.test('should log warning if not in main thread and make a stub api', (t) => {
    ...
    t.equal(
      loggerMock.warn.args[0][0],
      'Monis Agent for Node.js in worker_threads is not officially supported. Not starting! To bypass this, set `config.worker_threads.enabled` to true in configuration.'
    )
