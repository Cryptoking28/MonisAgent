...
function initialize() {
  ...
  try {
    ...
    if (!psemver.satisfies(pkgJSON.engines.node)) {
      message =
        `Monis Agent for Node.js requires a version of Node ${pkgJSON.engines.node}. \n` +
  ...
  if (agent) {
    ...
    if (agent.config.security.agent.enabled) {
      require('@monisagent/security-agent').start(api)
    }
  }
}
 ...
 *  1. -r monisagent
 *  2. --loader monisagent/esm-loader.mjs
 *  3. require('monisagent')
 *
