...
function getPlugin(shim, config) {
  return {
    applyToStack: (clientStack) => {
       ...
       * leverages the `build` step in it's logic. This caused x-monis-agent-disable-dt
       ...
       *
       * See:
       *   - https://github.com/Cryptoking28/monisagent/issues/1571
...
function headerMiddleware(shim, next) {
  return async function wrappedHeaderMw(args) {
    ...
    args.request.headers['x-monis-agent-disable-dt'] = 'true'
