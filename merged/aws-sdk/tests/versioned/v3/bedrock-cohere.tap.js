...
tap.test('successful text answer', async (t) => {
  ...
  const expected = responses.cohere.get(prompt)
...
tap.test('successful embed text answer', async (t) => {
  const { bedrock, client, responses } = t.context
...
tap.test('successful embed text answer (streamed)', async (t) => {
  const { bedrock, client, responses } = t.context
  const prompt = 'embed text stream'
  const command = new bedrock.InvokeModelWithResponseStreamCommand({
    body: JSON.stringify({
      input_type: 'search_document',
      texts: prompt.split(' ')
