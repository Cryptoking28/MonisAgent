...
const isFromEsmLoader = (context) =>
  context && context.parentURL && context.parentURL.includes('monisagent/esm-loader.mjs')

const logger = loggingModule.child({ component: 'esm-loader' })
const esmShimPath = new URL('./lib/esm-shim.mjs', import.meta.url)
const customEntryPoint = monisagent?.agent?.config?.api.esm.custom_instrumentation_entrypoint
...
addESMSupportabilityMetrics(monisagent.agent)
...
export async function resolve(specifier, context, nextResolve) {
  if (!monisagent.agent || !isSupportedVersion() || isFromEsmLoader(context)) {
  ...
  if (instrumentationDefinition) {
    ...
    if (registeredSpecifiers.get(url)) {
      ...
      // Keep track of what we've registered so we don't double register (see: https://github.com/Cryptoking28/monisagent/issues/1646)
 ...
 *
 * @param {object} agent
 *        instantiation of the Monis Agent agent
 * @returns {void}
 */
function addESMSupportabilityMetrics(agent) {
  ...
  if (isSupportedVersion()) {
    ...
    logger.warn(
      'Monis Agent for Node.js ESM loader requires a version of Node >= v16.12.0; your version is %s.  Instrumentation will not be registered.',
      process.version
    )
